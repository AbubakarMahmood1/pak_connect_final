...
void main() {
 ...
 locator.registerSingleton<ISharedMessageQueueProvider>(const _NoopSharedQueueProvider(),
);
 BLEMessageHandlerFacadeImpl.configureDependencyResolvers(sharedQueueProviderResolver: () => const _NoopSharedQueueProvider(),
);
 logRecords = [];
 allowedSevere = {};
 ...
 bleStateProvider.overrideWith((ref) => const AsyncValue.data(BluetoothLowEnergyState.poweredOff),
),
 eagerBurstScanningProvider.overrideWith((ref) async => true),
 spyModeDetectedProvider.overrideWith((ref) => const AsyncValue<SpyModeInfo>.loading(),
),
 identityRevealedProvider.overrideWith((ref) => const AsyncValue<String>.loading(),
),
 themeModeProvider.overrideWith(() => _FixedThemeModeNotifier(fixedThemeMode ?? ThemeMode.system),
),
];

 final scope = ProviderScope(overrides: overrides,
 ...
 Future<Completer<void>> pumpAppWhileInitializing(WidgetTester tester, {
 FakeBleService? fakeBleService,
 ThemeMode? fixedThemeMode,
 }) async {
 ...
 testWidgets('onGenerateRoute returns null for unknown route', (tester,
) async {
 await pumpApp(tester);
 await tester.pump();

 final materialApp = tester.widget<MaterialApp>(find.byType(MaterialApp));
 final route = materialApp.onGenerateRoute!(const RouteSettings(name: '/unknown-route'),
);
 expect(route, isNull);
 });
 ...
 final materialApp = tester.widget<MaterialApp>(find.byType(MaterialApp));
 final route = materialApp.onGenerateRoute!(const RouteSettings(name: '/group-chat', arguments: 'test-group-id'),
);
 ...
 testWidgets('shows loading indicator during initialization', (tester,
) async {
 ...
 expect(find.text('Initializing enhanced security and power management...'),
 findsOneWidget,
);
 ...
 testWidgets('themeModeProvider override applies light mode', (tester,
) async {
 ...
 final initLogs = logRecords.where((l) => l.loggerName == 'AppWrapper' && l.message.contains('started'),
);
 ...
 testWidgets('error screen has error icon when status is error', (tester,
) async {
 ...
 Text('Failed to initialize enhanced messaging features.\nCheck logs for details.',
),
