...
void main() {
 ...
 testWidgets('unreadCountStream returns values from repo', (tester,
) async {
 ...
 testWidgets('editDisplayName returns current when facade returns null', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('determineConnectionStatus returns facade result', (tester,
) async {
 ...
 testWidgets('determineConnectionStatus with offline status', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('archiveChat delegates to facade and reloads', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('deleteChat delegates to facade and reloads', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('clearSearch resets searchQuery and triggers loadChats', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats error sets isLoading false and rethrows', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);

 repo.getAllChatsError = 'db failure';

 expect(() => ctrl.loadChats(reset: true),
 throwsException,
);
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats sets hasMore false when searching', (tester,
) async {
 repo.queueResponse(List.generate(50,
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem passes searchQuery when set', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem sort: online before offline when both have same time', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('_safeNotifyListeners is no-op after dispose', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ...
 // -----------------------------------------------------------------------
 testWidgets('sequential loadMoreChats appends pages correctly', (tester,
) async {
 // Page 1: 50 items
 repo.queueResponse(List.generate(50,
 (i) => _item(id: 'p1-$i',
 ...
 // -----------------------------------------------------------------------
 testWidgets('isChatPinned returns false for non-pinned chat', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem is no-op after dispose', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ctrl.dispose();
 ...
 // -----------------------------------------------------------------------
 testWidgets('unreadCountStream refreshes after loadChats', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ...
 // -----------------------------------------------------------------------
 testWidgets('isLoading is true initially and false after loadChats', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
