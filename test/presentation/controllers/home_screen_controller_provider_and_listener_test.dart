...
class _ChatsRepo implements IChatsRepository {
  ...
  void queueResponse(List<ChatListItem> chats) => _queuedResponses.add(chats);
...
Future<HomeScreenController> _pumpWithLoadingProviders(
  WidgetTester tester, {
  required _ChatsRepo repo,
  required _Facade facade,
  _FakeConnectionService? connService,
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        discoveredDevicesProvider.overrideWith(
          (ref) => const AsyncValue<List<Peripheral>>.loading(),
        ),
        discoveryDataProvider.overrideWith(
          (ref) => const AsyncValue<Map<String, DiscoveredEventArgs>>.loading(),
        ),
        connectionServiceProvider.overrideWithValue(cs),
      ],
      child: MaterialApp(
...
void main() {
  ...
  group('HomeScreenController – supplementary', () {
    ...
    // -----------------------------------------------------------------------
    testWidgets('handleDeviceSelected delays then reloads chats', (
      tester,
    ) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('global message listener triggers chat update', (tester) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('setupGlobalMessageListener catches provider errors', (
      tester,
    ) async {
      late HomeScreenController ctrl;
        ...
        ProviderScope(
          overrides: [
            ...
            connectionServiceProvider.overrideWith(
              (ref) => throw StateError('Service not configured in test'),
            ),
          ],
          child: MaterialApp(
            home: _Host(
    ...
    // -----------------------------------------------------------------------
    testWidgets('discovery listener triggers loadChats on new data', (
      tester,
    ) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('peripheral listener setup exits early on non-Android', (
      tester,
    ) async {
      final cs = _FakeConnectionService();
      final ctrl = await _pump(
    ...
    // -----------------------------------------------------------------------
    testWidgets('loadChats with loading providers hits orElse branches', (
      tester,
    ) async {
      repo.queueResponse([_item(id: 'or1', name: 'OrElse')]);
      final ctrl = await _pumpWithLoadingProviders(
        tester,
        repo: repo,
        facade: facade,
      );
    ...
    testWidgets('updateSingleChatItem with loading providers hits orElse', (
      tester,
    ) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('unreadCountStream is set after initialize', (tester) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('showArchiveConfirmation delegates to facade', (tester) async {
    ...
    testWidgets('showDeleteConfirmation delegates to facade', (tester) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('updateSingleChatItem inserts new chat at front', (
      tester,
    ) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('updateSingleChatItem no-op when updatedChats is empty', (
      tester,
    ) async {
    ...
    // -----------------------------------------------------------------------
    testWidgets('onSearchChanged with single space sets query without reload', (
      tester,
    ) async {
      final ctrl = await _pump(tester, repo: repo, facade: facade);
    ...
    // -----------------------------------------------------------------------
    testWidgets('onSearchChanged with query triggers loadChats', (
      tester,
    ) async {
      repo.queueResponse([]);
      final ctrl = await _pump(tester, repo: repo, facade: facade);
    ...
    // -----------------------------------------------------------------------
    testWidgets('homeScreenControllerProvider creates and initializes', (
      tester,
    ) async {
      late HomeScreenController ctrl;
      final cs = _FakeConnectionService();
