/// Supplementary tests for HomeScreenController
...
Future<HomeScreenController> _pump(WidgetTester tester, {
 ...
 child: MaterialApp(home: _P13Host(controllerBuilder: (context, ref) {
...
void main() {
 ...
 // -----------------------------------------------------------------------
 testWidgets('onSearchChanged with single space sets sentinel', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('onSearchChanged with real query sets searchQuery and loads', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('onSearchChanged with whitespace clears searchQuery', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadMoreChats appends results and updates offset', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem updates existing and re-sorts', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem inserts new chat at beginning', (tester,
) async {
 // Initial load
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem with empty response is no-op', (tester,
) async {
 repo.queueResponse([
 _item(id: 'a', name: 'Alice', time: DateTime.now()),
]);
 ...
 // -----------------------------------------------------------------------
 testWidgets('showArchiveConfirmation returns facade result', (tester,
) async {
 ...
 testWidgets('showDeleteConfirmation returns facade result', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem sorts online chats before offline', (tester,
) async {
 final now = DateTime.now();
 repo.queueResponse([
 ...
 repo.queueResponse([
 ChatListItem(chatId: const ChatId('offline-1'),
 contactName: 'Offline',
 ...
 // -----------------------------------------------------------------------
 testWidgets('updateSingleChatItem handles null lastMessageTime in sort', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats reset=false appends to existing list', (tester,
) async {
 repo.queueResponse(List.generate(50,
 ...
 // -----------------------------------------------------------------------
 testWidgets('after dispose, loadChats and onSearchChanged are no-ops', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ctrl.dispose();
