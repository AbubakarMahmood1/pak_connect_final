/// Additional ChatScreenController coverage
...
void main() {
 ...
 Future<ChatScreenController> buildController(WidgetTester tester, {
 ChatScreenConfig config = const ChatScreenConfig(chatId: 'chat-1',
 contactName: 'T',
 ...
 connectionService.currentSessionId = 'chat-1';
 final c = await buildController(tester,
 config: const ChatScreenConfig(chatId: 'my-chat',
 ...
 final c = await buildController(tester,
 config: const ChatScreenConfig(chatId: 'custom-id',
 contactPublicKey: 'pk',
),
);
 ...
 testWidgets('uses contactPublicKey when no chatId in config', (tester,
) async {
 connectionService.currentSessionId = 'sess';
 final c = await buildController(tester,
 ...
 testWidgets('falls back to sessionId when no publicKey', (tester,
) async {
 ...
 testWidgets('returns Unknown when non-repo and no connection info', (tester,
) async {
 connectionService.currentSessionId = 'sess';
 ...
 testWidgets('returns otherUserName from connection when non-repo', (tester,
) async {
 connectionService.currentSessionId = 'sess';
 ...
 connectionService.currentSessionId = 'chat-1';
 final c = await buildController(tester);
 c.publishState(const ChatUIState(isLoading: false, initializationStatus: 'Done'),
);
 ...
 c.dispose();
 // Should not throw
 c.publishState(const ChatUIState(isLoading: false, initializationStatus: 'new'),
);
 });
 ...
 final c = await buildController(tester);
 // Should not throw
 c.handleMeshInitializationStatusChange(null,
 ...
 testWidgets('update with nonexistent message does not crash', (tester,
) async {
 connectionService.currentSessionId = 'chat-1';
 final c = await buildController(tester);
 final msg = Message(id: MessageId('nonexistent'),
 chatId: ChatId('chat-1'),
 content: 'ghost',
 timestamp: DateTime.now(),
 isFromMe: true,
 ...
 testWidgets('handleConnectionChange offline to offline', (tester,
) async {
 ...
 group('deleteMessage', () {
 testWidgets('deleteMessage with deleteForEveryone=true', (tester,
) async {
 connectionService.currentSessionId = 'chat-1';
 final c = await buildController(tester);
 await tester.runAsync(() async {
 ...
 testWidgets('deleteMessage with deleteForEveryone=false', (tester,
) async {
 connectionService.currentSessionId = 'chat-1';
 final c = await buildController(tester);
 await tester.runAsync(() async {
 ...
 testWidgets('handleConnectionChange after init with reconnect', (tester,
) async {
 connectionService.currentSessionId = 'sess-1';
 final c = await buildController(tester,
...
class _FakeMeshNetworkingService implements IMeshNetworkingService {
 ...
 @override
 Future<bool> setPriority(String messageId,
