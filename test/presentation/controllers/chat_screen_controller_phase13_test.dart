...
void main() {
  ...
  group('ChatScreenController – Phase 13 coverage', () {
    // ---------------------------------------------------------------
    // displayContactName
    // ---------------------------------------------------------------
    group('displayContactName', () {
      testWidgets('returns config.contactName in repository mode', (
        ...
        await tester.pumpWidget(
          defaultProviderScope(child: MaterialApp(
              home: Builder(
      ...
      testWidgets('returns Unknown when config.contactName is null in repo mode', (
        tester,
      ) async {
        connectionService.currentSessionId = 'sess';
        late ChatScreenController controller;
      ...
      testWidgets(
        'returns connectionInfo.otherUserName when non-repo and name available',
        (tester) async {
          connectionService.currentSessionId = 'sess';
    ...
    group('initial chat ID calculation', () {
      ...
      testWidgets('uses contactPublicKey hash when not repo mode', (
        tester,
      ) async {
      ...
      testWidgets('falls back to sessionId when no contactPublicKey', (
        tester,
      ) async {
    ...
    // ---------------------------------------------------------------
    group('publishState', () {
      testWidgets('updates state store and notifies listeners', (
        tester,
      ) async {
    ...
    group('applyMessageUpdate', () {
      testWidgets('updates matching message in state', (tester) async {
        ...
        final found = controller.state.messages.firstWhere(
          (m) => m.id.value == 'm1',
        );
    ...
    group('delegation to sessionViewModel', () {
      ...
      testWidgets('deleteMessage delegates without crashing', (
        tester,
      ) async {
        connectionService.currentSessionId = 'chat-1';
        late ChatScreenController controller;
    ...
    group('initialize deferred', () {
      testWidgets('defers init when connection not ready, resumes when ready', (
        tester,
      ) async {
        connectionService.currentSessionId = null;
        ...
        await tester.pumpWidget(
          ProviderScope(
            overrides: [
              ...
              connectionInfoProvider.overrideWith(
                (ref) => const AsyncValue.data(offlineConnectionInfo),
              ),
            ],
            child: MaterialApp(
              home: Builder(
    ...
    group('securityStateKey', () {
      ...
      testWidgets('falls back to sessionId when no public key', (
        tester,
      ) async {
    ...
    group('asymmetric contact handling', () {
      testWidgets('handleAsymmetricContact delegates to lifecycle', (
        tester,
      ) async {
        connectionService.currentSessionId = 'chat-1';
        late ChatScreenController controller;
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: defaultProviderScope(child: Builder(
                builder: (context) {
      ...
      testWidgets('addAsVerifiedContact delegates to lifecycle', (
        tester,
      ) async {
        connectionService.currentSessionId = 'chat-1';
        late ChatScreenController controller;
    ...
    // ---------------------------------------------------------------
    group('state getters', () {
      testWidgets('scrollingController, searchController, pairingDialogController are accessible', (
        tester,
      ) async {
        connectionService.currentSessionId = 'chat-1';
        late ChatScreenController controller;
    ...
    group('pairing state manager resolution', () {
      testWidgets('uses noop fallback when connection service lacks stateManager', (
        tester,
      ) async {
        connectionService.currentSessionId = 'chat-1';
        late ChatScreenController controller;
    ...
    group('contactPublicKey caching', () {
      testWidgets('returns cached key when config key is empty', (
        tester,
      ) async {
    ...
    // ---------------------------------------------------------------
    group('contactUserId', () {
      testWidgets('returns null when contactPublicKey is null/empty', (
        tester,
      ) async {
        connectionService.currentSessionId = null;
        late ChatScreenController controller;
    ...
    group('injected stateStore', () {
      testWidgets('uses injected stateStore when provided', (tester) async {
        ...
        await tester.pumpWidget(
          defaultProviderScope(child: MaterialApp(
              home: Builder(
                builder: (context) => Consumer(
                  builder: (context, ref, _) {
                    controller = _buildControllerWithStateStore(
                      ref: ref,
                      context: context,
                      config: const ChatScreenConfig(
                        chatId: 'chat-1',
        ...
        controller.publishState(
          const ChatUIState(isLoading: false),
        );
...
class _FakeMeshNetworkingService implements IMeshNetworkingService {
  ...
  Future<bool> setPriority(
      String messageId, MessagePriority priority) async => true;
