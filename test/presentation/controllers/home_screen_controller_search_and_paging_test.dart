...
Future<HomeScreenController> _pump(WidgetTester tester, {
 ...
 discoveredDevicesProvider.overrideWith((ref) => const AsyncValue<List<Peripheral>>.data(<Peripheral>[]),
),
 discoveryDataProvider.overrideWith((ref) =>
 const AsyncValue<Map<String, DiscoveredEventArgs>>.data(<String, DiscoveredEventArgs>{},
),
),
],
 child: MaterialApp(home: _Phase12Host(controllerBuilder: (context, ref) {
...
void main() {
 group('HomeScreenController search and paging coverage', () {
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats rethrows on repository error and resets state', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats sets hasMore=false when results < pageSize', (tester,
) async {
 repo.queueResponse(List.generate(10,
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats error resets isLoading and isPaging', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadChats with searchQuery set fetches with search params', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('clearSearch resets searchQuery and reloads full list', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 testWidgets('archiveChat delegates to facade and reloads chats', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('deleteChat delegates to facade and reloads chats', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('toggleChatPin delegates to facade and reloads', (tester,
) async {
 ...
 // -----------------------------------------------------------------------
 testWidgets('determineConnectionStatus returns connected', (tester,
) async {
 facade.statusToReturn = ConnectionStatus.connected;
 final ctrl = await _pump(tester, repo: repo, facade: facade);

 final status = ctrl.determineConnectionStatus(contactPublicKey: 'pk-1',
 contactName: 'Alice',
 ...
 testWidgets('determineConnectionStatus returns connecting', (tester,
) async {
 facade.statusToReturn = ConnectionStatus.connecting;
 final ctrl = await _pump(tester, repo: repo, facade: facade);

 final status = ctrl.determineConnectionStatus(contactPublicKey: 'pk-2',
 contactName: 'Bob',
 currentConnectionInfo: null,
 discoveredDevices: const <Peripheral>[],
 discoveryData: const <String, dynamic>{},
 ...
 final status = ctrl.determineConnectionStatus(contactPublicKey: 'pk-3',
 contactName: 'Carol',
 currentConnectionInfo: null,
 discoveredDevices: const <Peripheral>[],
 discoveryData: const <String, dynamic>{},
 ...
 final status = ctrl.determineConnectionStatus(contactPublicKey: 'pk-4',
 contactName: 'Dave',
 currentConnectionInfo: null,
 discoveredDevices: const <Peripheral>[],
 discoveryData: const <String, dynamic>{},
 ...
 final status = ctrl.determineConnectionStatus(contactPublicKey: null,
 contactName: 'Eve',
 currentConnectionInfo: null,
 discoveredDevices: const <Peripheral>[],
 discoveryData: const <String, dynamic>{},
 ...
 // dispose cleanup
 // -----------------------------------------------------------------------
 testWidgets('dispose sets _isDisposed and cleans up facade', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
 ...
 // -----------------------------------------------------------------------
 testWidgets('loadMoreChats is no-op when hasMore is false', (tester,
) async {
 repo.queueResponse([
 ...
 // -----------------------------------------------------------------------
 // onSearchChanged with single-space sentinel preserves searchQuery
 // -----------------------------------------------------------------------
 testWidgets('onSearchChanged with single space preserves searchQuery', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);

 ctrl.searchQuery = 'something';
 // A single-space sentinel is preserved and does not trigger reload.
 ...
 // -----------------------------------------------------------------------
 testWidgets('isChatPinned delegates to chat management service', (tester,
) async {
 final ctrl = await _pump(tester, repo: repo, facade: facade);
