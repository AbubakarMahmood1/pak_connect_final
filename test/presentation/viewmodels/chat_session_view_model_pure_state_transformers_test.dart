/// Supplementary tests for ChatSessionViewModel
...
class _FakeMessageRepo extends Fake implements IMessageRepository {
 ...
 storedMessages.cast<Message?>().firstWhere((m) => m?.id == messageId,
 orElse: () => null,
);
...
ChatSessionViewModel _buildVM({
 ...
 final scrollCtl = chat_controller.ChatScrollingController(chatsRepository: cr,
 chatId: _cid('chat-1'),
 onScrollToBottom: () {},
 ...
 return ChatSessionViewModel(config: config ?? const ChatScreenConfig(chatId: 'chat-1', contactName: 'Alice'),
...
void main() {
 ...
 final vm = _buildVM();
 const state = ChatUIState();
 final updated = vm.applyMeshState(state,
 ...
 final vm = _buildVM(onControllersRebound: ({
 required ChatMessagingViewModel messagingViewModel,
 required chat_controller.ChatScrollingController scrollingController,
 required ChatSearchController searchController,
 ChatMessagingViewModel? previousMessagingViewModel,
 ...
 final newScrollCtl = chat_controller.ChatScrollingController(chatsRepository: newChatsRepo,
 chatId: _cid('chat-2'),
 onScrollToBottom: () {},
 onUnreadCountChanged: (_) {},
 onStateChanged: () {},
);
 final newSearchCtl = ChatSearchController(onSearchModeToggled: (_) {},
 onSearchResultsChanged: (_, _) {},
 onNavigateToResult: (_) {},
 scrollController: newScrollCtl.scrollController,
);

 vm.rebindControllers(messagingViewModel: newMessaging,
 ...
 final vm = _buildVM(config: const ChatScreenConfig(chatId: 'repo-chat', contactName: 'X'),
);
 ...
 final vm = _buildVM(config: const ChatScreenConfig(contactName: 'X'),
 getChatIdFn: () => 'fn-chat',
);
 final result = vm.calculateInitialChatId();
