/// Supplementary tests for ChatMessagingViewModel
...
void main() {
 ...
 await vm.loadMessages(onScrollToBottom: () => scrollCalled = true,
 ...
 await expectLater(vm.loadMessages(onError: (msg) => errorMsg = msg,
 onLoadingStateChanged: (v) => loadingStates.add(v),
),
 throwsException,
);
 ...
 final vm = _buildViewModel(contactRepository: contactRepo,
 queueProvider: qp,
 ...
 String? errorMsg;
 await expectLater(vm.sendMessage(content: 'test',
 onShowError: (msg) => errorMsg = msg,
),
 ...
 Message? addedMsg;
 await vm.sendMessage(content: 'hello world',
 ...
 String? successMsg;
 MessageId? removedId;
 await vm.deleteMessage(messageId: const MessageId('del-1'),
 ...
 String? errorMsg;
 await vm.deleteMessage(messageId: const MessageId('del-2'),
 onShowError: (msg) => errorMsg = msg,
);
 ...
 String? errorMsg;
 await expectLater(vm.deleteMessage(messageId: const MessageId('del-3'),
 onShowError: (msg) => errorMsg = msg,
),
 throwsException,
);
...
ChatMessagingViewModel _buildViewModel({
 ...
 return ChatMessagingViewModel(chatId: const ChatId('chat-1'),
...
class _FakeMessageRepository implements IMessageRepository {
 ...
 messages.cast<Message?>().firstWhere((m) => m?.id == messageId,
 orElse: () => null,
);
 @override
