/// MeshNetworkingProvider coverage for uncovered lines.
///
...
void main() {
  ...
  group('Bluetooth bridge providers', () {
    ...
    test('bluetoothStatusMessageStreamProvider emits messages', () async {
      ...
      final sub = container.listen(
        bluetoothStatusMessageStreamProvider,
        (_, _) {},
      );
      addTearDown(sub.close);
    ...
    test(
      'bluetoothStatusMessageProvider wraps stream provider value',
      () async {
        final container = btContainer();

        final sub = container.listen(bluetoothStatusMessageProvider, (_, _) {});
        addTearDown(sub.close);

        fakeConnection.btMsgCtrl.add(BluetoothStatusMessage.disabled('BT off'));
        await Future<void>.delayed(const Duration(milliseconds: 50));
    ...
    test(
      'bluetoothReadyProvider returns true when stream reports ready',
      () async {
        final container = btContainer();
    ...
    test('bluetoothReadyProvider falls back to service.isBluetoothReady '
        'when no stream data', () {
  ...
  group('meshNetworkingServiceProvider', () {
    ...
    test('resolves from the service registry when registered', () async {
    ...
    test('throws when not registered — loading bootstrap', () async {
      ...
      final container = ProviderContainer(
        overrides: [
          appBootstrapProvider.overrideWith(() => _LoadingBootstrapNotifier()),
        ],
      );
      addTearDown(container.dispose);
  ...
  group('Binary payload providers', () {
    ...
    test('binaryPayloadStreamProvider emits events', () async {
      ...
      final sub = container.listen(binaryPayloadStreamProvider, (_, _) {});
      addTearDown(sub.close);
  ...
  group('Stream bridge providers', () {
    ...
    test('meshStatusStreamProvider emits status events', () async {
      ...
      final sub = container.listen(meshStatusStreamProvider, (_, _) {});
      addTearDown(sub.close);
    ...
    test('relayStatsStreamProvider emits relay stats', () async {
      ...
      final sub = container.listen(relayStatsStreamProvider, (_, _) {});
      addTearDown(sub.close);
    ...
    test('queueStatsStreamProvider emits queue sync stats', () async {
      ...
      final sub = container.listen(queueStatsStreamProvider, (_, _) {});
      addTearDown(sub.close);
  ...
  group('MeshRuntimeState', () {
    ...
    test('copyWith overrides individual fields', () {
      ...
      final withRelay = base.copyWith(relayStatistics: _defaultRelayStats);
      expect(withRelay.relayStatistics!.totalRelayed, 10);
      expect(withRelay.queueStatistics, isNull);

      final withQueue = base.copyWith(queueStatistics: _defaultQueueSyncStats);
