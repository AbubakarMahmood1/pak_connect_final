...
class _FakeMeshNetworkingService implements IMeshNetworkingService {
 ...
 MeshNetworkStatistics stats = MeshNetworkStatistics(nodeId: 'node-a',
 isInitialized: true,
 relayStatistics: const RelayStatistics(totalRelayed: 12,
 totalDropped: 1,
 ...
 queueStatistics: const QueueStatistics(totalQueued: 20,
 totalDelivered: 18,
 totalFailed: 2,
 pendingMessages: 1,
 sendingMessages: 0,
 ...
 syncStatistics: const QueueSyncManagerStats(totalSyncRequests: 4,
 successfulSyncs: 3,
 failedSyncs: 1,
...
MeshNetworkStatus _status({
 ...
 return MeshNetworkStatus(isInitialized: initialized,
...
void main() {
 group('mesh_networking_provider ', () {
 test('MeshNetworkingController handles send + sync success and failures',
 () async {
 final fake = _FakeMeshNetworkingService();
 final container = ProviderContainer(overrides: [meshNetworkingServiceProvider.overrideWithValue(fake)],
);
 addTearDown(container.dispose);

 final controller = container.read(meshNetworkingControllerProvider);

 final sent = await controller.sendMeshMessage(content: 'hello',
 ...
 final failedSend = await controller.sendMeshMessage(content: 'hello',
 recipientPublicKey: 'peer-key',
);
 ...
 final fake = _FakeMeshNetworkingService()
 ..stats = MeshNetworkStatistics(nodeId: 'node-z',
 isInitialized: false,
 relayStatistics: const RelayStatistics(totalRelayed: 1,
 totalDropped: 4,
 ...
 queueStatistics: const QueueStatistics(totalQueued: 30,
 totalDelivered: 10,
 totalFailed: 20,
 pendingMessages: 12,
 sendingMessages: 2,
 ...
 final uiState = MeshNetworkingUIState(networkStatus: AsyncValue.data(_status(initialized: true, connected: true),
),
 relayStats: const AsyncValue.data(RelayStatistics(totalRelayed: 7,
 totalDropped: 2,
 ...
 final event = ReceivedBinaryEvent(fragmentId: 'f1',
 originalType: 0x90,
 ...
 test('provider bridge ingests binary stream and pending transfers',
 () async {
 final fake = _FakeMeshNetworkingService()
 ...
 final event = ReceivedBinaryEvent(fragmentId: 'f2',
 originalType: 0x90,
