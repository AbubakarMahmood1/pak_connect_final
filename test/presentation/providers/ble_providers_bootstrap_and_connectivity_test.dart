...
class _StagedBootstrapNotifier extends AppBootstrapNotifier {
  _StagedBootstrapNotifier(this.readyFuture);
  ...
  Future<AppBootstrapState> build() async {
    state = const AsyncValue.data(
      AppBootstrapState(status: AppBootstrapStatus.initializing),
    );
    await readyFuture;
    return const AppBootstrapState(status: AppBootstrapStatus.ready);
  }
...
MeshNetworkStatus _meshStatus() {
  return const MeshNetworkStatus(
    isInitialized: true,
...
void main() {
  group('ble_providers ', () {
    setUp(clearRuntimeAppServicesForTesting);
    tearDown(clearRuntimeAppServicesForTesting);

    test('appBootstrapReadyProvider waits for ready state', () async {
      final readyCompleter = Completer<void>();
      final container = ProviderContainer(
        overrides: [
          appBootstrapProvider.overrideWith(
            () => _StagedBootstrapNotifier(readyCompleter.future),
          ),
        ],
      );
      addTearDown(container.dispose);

      final future = container.read(appBootstrapReadyProvider.future);
      await Future<void>.delayed(const Duration(milliseconds: 20));

      expect(
        container.read(appBootstrapProvider).asData?.value.status,
        AppBootstrapStatus.initializing,
      );

      readyCompleter.complete();
      final readyState = await future;
      expect(readyState.isReady, isTrue);
    });

    test(
      'bleServiceInitializedProvider waits for bootstrap readiness',
      () async {
        ...
        final future = container.read(bleServiceInitializedProvider.future);
        await Future<void>.delayed(const Duration(milliseconds: 20));
    ...
    test(
      'bleServiceInitializedProvider initializes uninitialized services',
      () async {
        final service = _InitializableConnectionService();
        final container = ProviderContainer(
          overrides: [
        ...
        final resolved = await container.read(
          bleServiceInitializedProvider.future,
        );
        expect(resolved, same(service));
        expect(service.initializeCalls, 1);
        expect(service.isInitialized, isTrue);
    ...
    test(
      'serverConnectionsStreamProvider forwards manager stream values',
      () async {
        ...
        final serviceWithManager = _ConnectionServiceWithManager(
          _ConnectionManagerWithStream(Stream.value(connections)),
        );
        ...
        final sub = container.listen<AsyncValue<List<BLEServerConnection>>>(
          serverConnectionsStreamProvider,
          (previous, next) {
            next.whenData((value) {
      ...
      final container = ProviderContainer(
        overrides: [
          ...
          meshNetworkStatusProvider.overrideWith(
            (ref) => AsyncValue.data(_meshStatus()),
          ),
          bleStateProvider.overrideWith(
            (ref) => const AsyncValue.data(BluetoothLowEnergyState.poweredOn),
          ),
