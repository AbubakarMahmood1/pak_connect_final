...
void main() {
 ...
 test('LOW security with persistent-only uses persistent as last resort',
 () async {
 final contactRepo = _FakeContactRepository()
 ..contact = _contact(publicKey: '',
 ...
 test('MEDIUM security without persistent but with ephemeral uses ephemeral',
 () async {
 final contactRepo = _FakeContactRepository()
 ..contact = _contact(publicKey: 'pk1',
 ephemeral: 'eph-key',
 ...
 test('getContact throws falls back to contactPublicKey',
 () async {
 final contactRepo = _FakeContactRepository()
 ...
 bool scrolled = false;
 String? successMsg;
 await vm.sendMessage(content: 'reply msg',
 onScrollToBottom: () {
 scrolled = true;
 },
 onShowSuccess: (msg) {
 ...
 final messages = await vm.loadMessages(onGetQueuedMessages: () => [],
);
 ...
 final messages = await vm.loadMessages(onGetQueuedMessages: () => queued,
);
