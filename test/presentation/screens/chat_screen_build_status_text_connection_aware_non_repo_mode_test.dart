...
class _FakeContactRepository extends Fake implements IContactRepository {
 ...
 @override
 Future<void> updateContactEphemeralId(String pk,
 ...
 @override
 Future<void> downgradeSecurityForDeletedContact(String pk,
 String reason,
) async {}
...
const _defaultConnectionInfo = ConnectionInfo(isConnected: true,
 isReady: true,
...
const _reconnectingInfo = ConnectionInfo(isConnected: false,
 isReady: false,
 isReconnecting: true,
);

const _defaultMeshStatus = MeshNetworkStatus(isInitialized: true,
 isConnected: true,
 statistics: MeshNetworkStatistics(nodeId: 'test-node',
 isInitialized: true,
...
SecurityState _verifiedSecurityState() => const SecurityState(status: SecurityStatus.verifiedContact,
...
SecurityState _pairedSecurityState() => const SecurityState(status: SecurityStatus.paired,
 canSendMessages: true,
 showContactAddButton: false,
 showPairingButton: false,
 showContactSyncButton: false,
...
SecurityState _needsPairingSecurityState() => const SecurityState(status: SecurityStatus.needsPairing,
 canSendMessages: true,
 showContactAddButton: false,
...
SecurityState _asymmetricSecurityState() => const SecurityState(status: SecurityStatus.asymmetricContact,
 canSendMessages: true,
 showContactAddButton: false,
 showPairingButton: false,
...
SecurityState _disconnectedSecurityState() => const SecurityState(status: SecurityStatus.disconnected,
...
SecurityState _showAddContactState() => const SecurityState(status: SecurityStatus.needsPairing,
 canSendMessages: true,
...
SecurityState _unknownSecurityState() => const SecurityState(status: SecurityStatus.unknown,
 canSendMessages: false,
 showContactAddButton: false,
 showPairingButton: false,
 showContactSyncButton: false,
...
Future<void> _pumpChatScreen(WidgetTester tester, {
 ...
 connectionInfoProvider.overrideWith((ref) => AsyncValue.data(connectionInfo),
),
 meshNetworkingServiceProvider.overrideWith((ref) => meshService),
 meshNetworkStatusProvider.overrideWith((ref) => const AsyncValue.data(_defaultMeshStatus),
),
 securityStateProvider.overrideWith((ref, key) async => secState),
 binaryPayloadStreamProvider.overrideWith((ref) => meshService.binaryPayloadStream,
),
 if (binaryInbox != null)
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: chatId,
...
// ============================================================================
// TESTS — Comprehensive uncovered-line coverage
// ============================================================================
...
void main() {
 ...
 testWidgets('shows "Connected • ECDH Encrypted" wording in repo mode', (tester,
) async {
 ...
 testWidgets('tapping pairing button (lock_open) does not crash', (tester,
) async {
 await _pumpChatScreen(tester,
 securityState: _needsPairingSecurityState(),
);
 ...
 testWidgets('tapping add-contact button (person_add) does not crash', (tester,
) async {
 ...
 testWidgets('tapping search icon toggles mode without crash', (tester,
) async {
 await _pumpChatScreen(tester);
 ...
 testWidgets('hides banner when connected and not reconnecting', (tester,
) async {
 await _pumpChatScreen(tester,
 connectionInfo: _defaultConnectionInfo,
 ...
 final reconnectButtons = find.descendant(of: bannerFinder,
 ...
 PendingBinaryTransfer(transferId: 'tx-1',
 recipientId: 'r-1',
 originalType: 1,
),
];
 ...
 testWidgets('tapping retry on pending banner does not crash', (tester,
) async {
 final meshSvc = _FakeMeshNetworkingService();
 meshSvc.pendingTransfers = [
 PendingBinaryTransfer(transferId: 'tx-2',
 recipientId: 'r-2',
 originalType: 1,
),
];
 ...
 expect(bannerFinder, findsOneWidget);
 final tapTargets = find.descendant(of: bannerFinder,
 matching: find.byType(InkWell),
);
 ...
 final event = ReceivedBinaryEvent(fragmentId: 'frag-1',
 originalType: 1,
 ...
 testWidgets('shows "Loading..." when security state pending', (tester,
) async {
 ...
 await tester.pumpWidget(ProviderScope(overrides: [
 connectionServiceProvider.overrideWith((ref) => connService),
 connectionInfoProvider.overrideWith((ref) => const AsyncValue.data(_defaultConnectionInfo),
),
 meshNetworkingServiceProvider.overrideWith((ref) => meshService),
 meshNetworkStatusProvider.overrideWith((ref) => const AsyncValue.data(_defaultMeshStatus),
),
 securityStateProvider.overrideWith((ref, key) => neverCompletes.future,
),
 binaryPayloadStreamProvider.overrideWith((ref) => meshService.binaryPayloadStream,
),
 chatScreenControllerProvider.overrideWith((ref, args) {
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'load-chat',
 ...
 testWidgets('shows "Error" with red when security state fails', (tester,
) async {
 final meshService = _FakeMeshNetworkingService();
 final connService = _FakeConnectionService();
 final msgRepo = _FakeMessageRepository();
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'err-chat',
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'l-chat',
 contactName: 'LUser',
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'err2-chat',
 ...
 testWidgets('AppBar has Column with contact name and status', (tester,
) async {
 await _pumpChatScreen(tester,
 contactName: 'Zara',
 securityState: _pairedSecurityState(),
 ...
 testWidgets('AppBar shows search and action buttons together', (tester,
) async {
 await _pumpChatScreen(tester,
 securityState: _needsPairingSecurityState(),
);
 ...
 testWidgets('image icon present when disconnected (composer renders)', (tester,
) async {
 await _pumpChatScreen(tester, connectionInfo: _disconnectedInfo);
 ...
 testWidgets('securityStateKey is derived from contactPublicKey', (tester,
) async {
 await _pumpChatScreen(tester,
 ...
 final meshSvc = _FakeMeshNetworkingService();
 meshSvc.pendingTransfers = [
 PendingBinaryTransfer(transferId: 'tx-a',
 recipientId: 'r-a',
 originalType: 1,
),
 PendingBinaryTransfer(transferId: 'tx-b',
 recipientId: 'r-b',
 originalType: 2,
),
];
 ...
 testWidgets('widget initializes subscriptions without crash', (tester,
) async {
 await _pumpChatScreen(tester);
 ...
 testWidgets('resolves ISecurityService via registry without crash', (tester,
) async {
 final secService = _FakeSecurityService();
 _registerService<ISecurityService>(locator, secService);
