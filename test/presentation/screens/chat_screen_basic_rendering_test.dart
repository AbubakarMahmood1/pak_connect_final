...
void _cleanupRegistry(TestServiceRegistry locator) {
...
void main() {
  ...
  group('ChatScreen – security async states', () {
    ...
    testWidgets('renders correctly with paired security state', (tester) async {
      await _pumpChatScreen(tester, securityState: _pairedSecurityState());
  ...
  group('ChatScreen – AppBar details', () {
    ...
    testWidgets('AppBar subtitle uses grey for loading state', (tester) async {
      final locator = serviceRegistry;
      final msgRepo = _FakeMessageRepository();
      final contactRepo = _FakeContactRepository();
      final chatsRepo = _FakeChatsRepository();
