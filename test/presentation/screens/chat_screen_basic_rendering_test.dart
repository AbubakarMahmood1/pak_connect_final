...
class _FakeContactRepository extends Fake implements IContactRepository {
 ...
 @override
 Future<bool> upgradeContactSecurity(String pk,
 SecurityLevel level,
) async =>
 true;
...
class _FakeConnectionService extends Fake implements IConnectionService {
 ...
 @override
 ConnectionInfo get currentConnectionInfo => const ConnectionInfo(isConnected: true,
 isReady: true,
);
...
const _disconnectedInfo = ConnectionInfo(isConnected: false,
 isReady: false,
);
...
Future<void> _pumpChatScreen(WidgetTester tester, {
 ...
 final connService = _FakeConnectionService(activelyReconnecting: isActivelyReconnecting,
);
...
void main() {
 ...
 final textWidget = tester.widget<Text>(find.textContaining('ECDH Encrypted'),
);
 ...
 final textWidget = tester.widget<Text>(find.textContaining('Paired'),
);
 ...
 final textWidget = tester.widget<Text>(find.textContaining('Contact Sync Needed'),
);
 ...
 final textWidget = tester.widget<Text>(find.textContaining('Basic Encryption'),
);
 expect(textWidget.style?.color, Colors.orange);
 });
 ...
 final textWidget = tester.widget<Text>(find.textContaining('Disconnected'),
);
 ...
 testWidgets('shows "Loading..." when security state resolves', (tester,
) async {
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'test-chat',
 contactName: 'Loader',
 ...
 testWidgets('shows no action button during security loading', (tester,
) async {
 final locator = getIt;
 final msgRepo = _FakeMessageRepository();
 final contactRepo = _FakeContactRepository();
 ...
 testWidgets('entering text and tapping send clears the field', (tester,
) async {
 await _pumpChatScreen(tester);
 ...
 testWidgets('AppBar subtitle uses grey for loading state', (tester,
) async {
 final locator = getIt;
 final msgRepo = _FakeMessageRepository();
 final contactRepo = _FakeContactRepository();
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'grey-chat',
 ...
 child: MaterialApp(home: ChatScreen.fromChatData(chatId: 'red-chat',
 contactName: 'RedUser',
