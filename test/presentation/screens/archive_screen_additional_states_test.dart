...
// loading/error states for search, archive detail navigation, FAB visibility,
...
import 'package:pak_connect/presentation/screens/archive_detail_screen.dart';
...
ArchivedChat _detailArchive({
  required String id,
  required String contactName,
  required String content,
}) {
  final archivedAt = _now.subtract(const Duration(days: 2));
  final message = Message(
    id: MessageId('detail_$id'),
    chatId: ChatId('chat_$id'),
    content: content,
  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        archivedChatProvider.overrideWith((ref, archiveId) async {
          return detailArchive?.id == archiveId ? detailArchive : null;
        }),
      ],
...
void main() {
  ...
  group('ArchiveScreen – ', () {
    ...
    // -----------------------------------------------------------------------
    // 10. Archive detail screen opens with persisted messages
    // -----------------------------------------------------------------------
    testWidgets('tapping archive tile opens real archived messages', (
      tester,
    ) async {
      final uiNotifier = _TestArchiveUINotifier(const ArchiveUIState());
      final operationsNotifier = _TestArchiveOperationsNotifier(
      ...
      final detailArchive = _detailArchive(
        id: 'a1',
        contactName: 'Fiona Detail',
        content: 'Persisted detail message from repository',
      );
      ...
      expect(find.byType(ArchiveDetailScreen), findsOneWidget);
      expect(find.text('Archived: Fiona Detail'), findsOneWidget);
      expect(
    ...
    // -----------------------------------------------------------------------
    // 17. Search result tile tap opens the owning archive
    // -----------------------------------------------------------------------
    testWidgets('tapping search result tile opens real archived messages', (
      tester,
    ) async {
      final uiNotifier = _TestArchiveUINotifier(
        const ArchiveUIState(isSearchMode: true, searchQuery: 'hello'),
      ...
      final detailArchive = _detailArchive(
        id: 'archive_1',
        contactName: 'Alice Detail',
        content: 'Persisted search-result archive message',
      );
      ...
      expect(find.byType(ArchiveDetailScreen), findsOneWidget);
      expect(find.text('Archived: Alice Detail'), findsOneWidget);
      expect(
        find.text('Persisted search-result archive message'),
        findsOneWidget,
      );
    });
