  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        discoveryScannerRecoveryProvider.overrideWith(
          (ref) =>
        ...
        deduplicatedDevicesProvider.overrideWith(
          (ref) =>
              dedupStream ?? Stream.value(const <String, DiscoveredDevice>{}),
        ),
...
void main() {
  ...
  group('DiscoveryOverlay _startScanning', () {
    ...
    testWidgets(
      '_startScanning with default recovery controller does not crash',
      (tester) async {
        await _pump(
          tester,
          controller: _StubController(DiscoveryOverlayState.initial()),
  ...
  group('DiscoveryOverlay _connectToDevice', () {
    ...
    testWidgets('successful connection dismisses dialog and notifies selection', (
      tester,
    ) async {
      final svc = MockConnectionService();
      final device = FakePeripheral(uuid: UUID.fromString(_deviceUuid));
      ...
      // Advance past Future.delayed(2s) to trigger UUID match branch
      // and onDeviceSelected.
      await tester.pump(const Duration(seconds: 3));
  ...
  group('DiscoveryOverlay gestures', () {
    testWidgets('swipe down on backdrop closes overlay', (tester) async {
      ...
      await tester.flingFrom(const Offset(10, 10), const Offset(0, 400), 1000);
      await tester.pump(const Duration(milliseconds: 500));
