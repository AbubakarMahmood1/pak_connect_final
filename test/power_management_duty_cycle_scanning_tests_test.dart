// Comprehensive tests for Duty Cycle Scanning & Emergency Mode
...
void main() {
 ...
 group('Duty Cycle Scanning Tests', () {
 setUp(() async {
 logRecords.clear();
 ...
 final perfStats = PowerManagementStats(currentScanInterval: 60000,
 ...
 final ultraLowStats = PowerManagementStats(currentScanInterval: 60000,
 currentHealthCheckInterval: 30000,
 consecutiveSuccessfulChecks: 10,
 consecutiveFailedChecks: 0,
 connectionQualityScore: 0.9,
 ...
 expect(ultraLowStats.batteryEfficiencyRating,
 ...
 debugPrint(' Performance mode: ${(perfStats.batteryEfficiencyRating * 100).toStringAsFixed(1)}%',
);
 debugPrint(' Ultra Low Power: ${(ultraLowStats.batteryEfficiencyRating * 100).toStringAsFixed(1)}%',
);
 });
 ...
 debugPrint(' ${powerManager.currentPowerMode.name}: ${powerManager.rssiThreshold} dBm',
);
 ...
 debugPrint(' Current mode (${powerManager.currentPowerMode.name}): ${powerManager.maxConnections} connections',
);
 expect(powerManager.maxConnections, greaterThan(0));
 });
 });

 group('Emergency Mode Sync Skipping Tests', () {
 ...
 syncManager = GossipSyncManager(myNodeId: 'test-node-id',
 ...
 debugPrint('✅ Normal mode: battery ${stats['batteryLevel']}% - emergency mode: ${stats['emergencyMode']}',
);
 });
 ...
 debugPrint('⚠️ Emergency mode: battery ${stats['batteryLevel']}% - emergency mode: ${stats['emergencyMode']}',
);
 });
 ...
 debugPrint('✅ Charging overrides emergency mode: battery ${stats['batteryLevel']}% charging: ${stats['isCharging']} - emergency mode: ${stats['emergencyMode']}',
);
 });
 ...
 group('Power Mode Transitions Tests', () {
 setUp(() async {
 logRecords.clear();
 Logger.root.level = Level.ALL;
 Logger.root.onRecord.listen(logRecords.add);
 ...
 debugPrint(' Initial (100% battery, foreground): ${powerManager.currentPowerMode.name}',
);
 ...
 debugPrint(' After background (100% battery, background): ${powerManager.currentPowerMode.name}',
);

 powerManager.setAppBackgroundState(false);
 // Back to performance
 expect(powerManager.currentPowerMode, PowerMode.performance);
 debugPrint(' After foreground (100% battery, foreground): ${powerManager.currentPowerMode.name}',
);
 });
 });
 ...
 final syncManager = GossipSyncManager(myNodeId: 'test-node',
 messageQueue: messageQueue,
);
...
extension PowerManagementStatsCopyWith on PowerManagementStats {
 ...
 return PowerManagementStats(currentScanInterval: currentScanInterval,
