...
void main() {
  ...
  group('getHealthStatus', () {
    ...
    test('critical when storage over limit and performance degraded', () async {
      await service.initialize();
  ...
  group('event listener error tolerance', () {
    test('archiveUpdate events are emitted to listeners', () async {
      ...
      expect(result.success, true);
      expect(
        events.any((e) => e.type == ArchiveUpdateEventType.archived),
        true,
      );
    ...
    test('policyUpdate events emitted on add and remove', () async {
      ...
      expect(events.any((e) => e.type == ArchivePolicyEventType.updated), true);
      expect(events.any((e) => e.type == ArchivePolicyEventType.removed), true);
  ...
  group('policy persistence', () {
    test('loads previously saved policies', () async {
      ...
      SharedPreferences.setMockInitialValues({
        'archive_policies_v2': jsonEncode(
          policies.map((p) => p.toJson()).toList(),
        ),
      });
      ...
      expect(svc.archivePolicies.any((p) => p.name == 'Saved Policy'), true);
      await svc.dispose();
    });
  ...
  group('archive policy management', () {
    test('adds new policy', () async {
      ...
      expect(service.archivePolicies.any((p) => p.name == 'NewPolicy'), true);
    });
    ...
    test('removes policy by name', () async {
      ...
      expect(service.archivePolicies.any((p) => p.name == 'ToRemove'), false);
    });
  });
  ...
  group('applyArchivePolicies', () {
    test('returns empty result on engine exception', () async {
      final engine = _StubPolicyEngine(repository)..throwOnApplyPolicies = true;
      final svc = ArchiveManagementService.withDependencies(
        archiveRepository: repository,
        policyEngine: engine,
      );
