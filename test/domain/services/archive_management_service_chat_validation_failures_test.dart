...
/// Supplementary tests for ArchiveManagementService
...
void main() {
 ...
 service = ArchiveManagementService.withDependencies(archiveRepository: repository,
);
 });
 ...
 ..archiveValidation = const ArchiveValidationResult(false,
 'Policy blocked this archive',
 ['Inactivity check failed'],
);
 final svc = ArchiveManagementService.withDependencies(archiveRepository: repository,
 policyEngine: engine,
);
 ...
 await service.updateConfiguration(const ArchiveManagementConfig(enableCompression: true,
 ...
 final result = await service.archiveChat(chatId: 'chat_force',
 force: true,
);
 ...
 final result = await svc.restoreChat(archiveId: archiveId,
 ...
 await service.updateArchivePolicy(ArchivePolicy.largeChats(name: 'Broken',
 ...
 () async {
 await service.initialize();
 repository.statistics = ArchiveStatistics(totalArchives: 10,
 totalMessages: 500,
 ...
 performanceStats: const ArchivePerformanceStats(averageArchiveTime: Duration(seconds: 5),
 ...
 await service.updateArchivePolicy(ArchivePolicy.byContact(name: 'EventPolicy',
 contactPattern: 'ev_*',
 enabled: true,
),
);
 ...
 expect(events.any((e) => e.type == ArchivePolicyEventType.updated),
 true,
);
 expect(events.any((e) => e.type == ArchivePolicyEventType.removed),
 true,
);
 ...
 await service.performMaintenance(tasks: {ArchiveMaintenanceTask.cleanupOrphaned},
);
 await Future<void>.delayed(const Duration(milliseconds: 30));

 expect(events.any((e) => e.type == ArchiveMaintenanceEventType.started),
 true,
);
 await sub.cancel();
 });
 ...
 final config = const ArchiveManagementConfig(enableCompression: false,
 ...
 final policies = [
 ArchivePolicy.byContact(name: 'Saved Policy',
 ...
 expect(svc.archivePolicies.any((p) => p.name == 'Saved Policy'),
 true,
);
 await svc.dispose();
 });
 ...
 await service.updateConfiguration(const ArchiveManagementConfig(enableCompression: false,
 ...
 await service.updateArchivePolicy(ArchivePolicy.byContact(name: 'NewPolicy',
 contactPattern: 'new_*',
 enabled: true,
),
);

 expect(service.archivePolicies.any((p) => p.name == 'NewPolicy'),
 true,
);
 });
 ...
 await service.updateArchivePolicy(ArchivePolicy.byContact(name: 'ReplaceMe',
 contactPattern: 'a_*',
 enabled: true,
),
);
 ...
 await service.updateArchivePolicy(ArchivePolicy.byContact(name: 'ToRemove',
 contactPattern: 'rm_*',
 enabled: true,
),
);
 ...
 expect(service.archivePolicies.any((p) => p.name == 'ToRemove'),
 false,
);
 });
 });
 ...
 expect(() => service.archiveChat(chatId: 'after_dispose'),
 ...
 expect(ArchiveManagementService.fromServiceLocator,
 throwsA(isA<StateError>()),
);
 });
 ...
 final custom = ArchiveManagementService.withDependencies(archiveRepository: customRepo,
);
 ...
 await service.performMaintenance(tasks: {ArchiveMaintenanceTask.rebuildIndex},
);
 await Future<void>.delayed(const Duration(milliseconds: 30));
...
ArchiveStatistics _statsWithSize(int totalSizeBytes) {
 return ArchiveStatistics(totalArchives: 2,
 totalMessages: 20,
 ...
 performanceStats: const ArchivePerformanceStats(averageArchiveTime: Duration(seconds: 1),
...
ArchiveStatistics _healthyStats() {
 return ArchiveStatistics(totalArchives: 3,
 totalMessages: 50,
 compressedArchives: 1,
 ...
 performanceStats: const ArchivePerformanceStats(averageArchiveTime: Duration(milliseconds: 500),
...
class _FakeArchiveRepository implements IArchiveRepository {
 ...
 return ArchiveOperationResult.failure(message: 'Missing archive',
...
class _StubPolicyEngine extends ArchivePolicyEngine {
 ...
 @override
 Future<ArchiveValidationResult> validateArchiveRequest(ChatId chatId,
 bool force,
) async =>
 archiveValidation;

 @override
 Future<ArchiveValidationResult> validateRestoreRequest(ArchivedChat archive,
 bool overwrite,
) async =>
 restoreValidation;

 @override
 Future<RestoreConflictCheck> checkRestoreConflicts(ArchivedChat archive,
 ChatId? targetChatId,
) async {
...
ArchivedChat _buildArchivedChat(ArchiveId archiveId, String chatId) {
 ...
 final message = Message(id: MessageId('msg_$chatId'),
 ...
 messageCount: 1,
 metadata: ArchiveMetadata(version: '1.0',
 reason: 'seed',
 ...
 messages: [
 ArchivedMessage.fromMessage(message,
 archivedAt,
