...
void main() {
 ...
 final summaries = await service.getEnhancedArchiveSummaries(limit: 10,
 offset: 0,
);
 ...
 await service.initialize();
 await service.updateArchivePolicy(ArchivePolicy.byContact(name: 'TestPolicy',
 contactPattern: 'test_*',
 enabled: true,
),
);
 final result = await service.applyArchivePolicies(specificPolicies: ['TestPolicy'],
 dryRun: false,
);
 expect(result, isNotNull);
 });
 ...
 await service.initialize();
 repository.statistics = _stats(totalArchives: 5,
 totalMessages: 100,
 ...
 await service.initialize();
 final analytics = await service.getArchiveAnalytics(since: DateTime(2025, 1, 1),
 ...
 const newConfig = ArchiveManagementConfig(enableCompression: false,
...
ArchiveStatistics _stats({
 ...
 return ArchiveStatistics(totalArchives: totalArchives,
 ...
 performanceStats: const ArchivePerformanceStats(averageArchiveTime: Duration.zero,
...
class _FakeArchiveRepository implements IArchiveRepository {
 ...
 ArchiveSearchResult.fromResults(messages: const [],
 chats: const [],
 query: query,
