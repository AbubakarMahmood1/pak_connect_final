// BurstScanningController coverage
...
void main() {
 ...
 controller.reportConnectionSuccess(rssi: -60,
 connectionTime: 150,
 ...
 controller.reportConnectionFailure(reason: 'connection_lost',
 rssi: -85,
 attemptTime: 3000,
);
 final stats = controller.getCurrentStatus().powerStats;
 ...
 await controller.triggerManualScan(delay: const Duration(milliseconds: 100),
);
 final status = controller.getCurrentStatus();
 ...
 expect(ratings[PowerMode.ultraLowPower]!,
 greaterThan(ratings[PowerMode.performance]!),
);
 expect(ratings[PowerMode.powerSaver]!,
 greaterThan(ratings[PowerMode.performance]!),
);
 });
 ...
 await controller.triggerManualScan(delay: const Duration(seconds: 5),
);
 final status = controller.getCurrentStatus();
 expect(status.secondsUntilNextScan, isNotNull);
 });
 ...
 await controller.triggerManualScan(delay: Duration.zero,
);
