...
///
/// Uses the generated mocks shared with the construction/initialization tests.
library;
...
// Import generated mocks shared with the construction/initialization tests.
...
void main() {
  ...
  group('Relay callbacks', () {
    test(
      'handleRelayDecision processes decision with long messageId',
      () async {
        ...
        final onRelayDecisionMade = captured[1] as void Function(RelayDecision);

        // Invoke with long messageId to cover truncation (line 543)
        final decision = RelayDecision.relayed(
          messageId: 'abcdefghijklmnopqrstuvwxyz1234567890',
    ...
    test(
      'handleRelayDecision processes decision with short messageId',
      () async {
        await initializeService(nodeId: 'test-node');
    ...
    test(
      'handleRelayStatsUpdated emits to health monitor and broadcasts',
      () async {
        await initializeService(nodeId: 'test-node');
  ...
  group('MessageHandler relay setter callbacks', () {
    test(
      'onRelayDecisionMade setter is called during BLE integration',
      () async {
    ...
    test(
      'onRelayStatsUpdated setter is called during BLE integration',
      () async {
  ...
  group('refreshMeshStatus', () {
    test(
      'refreshMeshStatus broadcasts status with nodeId after init',
      () async {
        ...
        final verification = verify(
          mockHealthMonitor.broadcastMeshStatus(
            isInitialized: captureAnyNamed('isInitialized'),
            currentNodeId: captureAnyNamed('currentNodeId'),
