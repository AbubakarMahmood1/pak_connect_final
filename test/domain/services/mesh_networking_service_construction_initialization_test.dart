...
void main() {
 ...
 verify(mockHealthMonitor.schedulePostFrameStatusUpdate(isInitialized: anyNamed('isInitialized'),
 ...
 verify(mockQueueCoordinator.initialize(nodeId: 'test-node-123',
 messageQueue: anyNamed('messageQueue'),
 onStatusChanged: anyNamed('onStatusChanged'),
)).called(1);
 });
 ...
 final result = await service.sendMeshMessage(content: 'hello world',
 recipientPublicKey: 'recipient-abc',
);

 expect(result.isSuccess, isTrue);
 expect(result.isRelay, isTrue);
 verify(mockRelayCoordinator.sendRelayMessage(content: 'hello world',
 recipientPublicKey: 'recipient-abc',
 chatId: anyNamed('chatId'),
 ...
 await service.sendMeshMessage(content: 'urgent message',
 recipientPublicKey: 'recipient-xyz',
 priority: MessagePriority.high,
);

 verify(mockRelayCoordinator.sendRelayMessage(content: 'urgent message',
 recipientPublicKey: 'recipient-xyz',
 chatId: anyNamed('chatId'),
 priority: MessagePriority.high,
)).called(1);
 ...
 when(mockBleService.retryBinaryMedia(transferId: anyNamed('transferId'),
 recipientId: anyNamed('recipientId'),
 originalType: anyNamed('originalType'),
)).thenAnswer((_) async => true);

 final result = await service.retryBinaryMedia(transferId: 'transfer-1',
 recipientId: 'recipient-1',
 originalType: 1,
);

 expect(result, isTrue);
 verify(mockBleService.retryBinaryMedia(transferId: 'transfer-1',
 recipientId: 'recipient-1',
 originalType: 1,
)).called(1);
 });
 ...
 final result = await service.sendMeshMessage(content: 'direct hello',
 recipientPublicKey: 'recipient-key',
);
 ...
 final syncResult = QueueSyncResult.success(messagesReceived: 1,
 messagesUpdated: 0,
 messagesSkipped: 0,
 finalHash: 'abc123',
 ...
 expect(result, isTrue);
 verify(mockQueueCoordinator.setPriority('msg-3', MessagePriority.urgent),
).called(1);
 });
 ...
 final result = await service.sendMeshMessage(content: 'test',
 ...
 verifyNever(mockQueueCoordinator.initialize(nodeId: 'node-2',
 messageQueue: anyNamed('messageQueue'),
 onStatusChanged: anyNamed('onStatusChanged'),
));
 });
