...
import 'mesh_networking_service_construction_initialization_test.mocks.dart';
...
void main() {
  ...
  void stubForInitialize() {
    ...
    when(
      mockRelayCoordinator.initialize(
        nodeId: anyNamed('nodeId'),
        messageQueue: anyNamed('messageQueue'),
    ...
    when(
      mockMessageHandler.initializeRelaySystem(
  ...
  setUp(() {
    ...
    when(
      mockRepositoryProvider.messageRepository,
    ).thenReturn(mockMessageRepository);

    // Stub health monitor streams
    when(
      mockHealthMonitor.meshStatus,
    ).thenAnswer((_) => Stream<MeshNetworkStatus>.empty());
    when(
      mockHealthMonitor.relayStats,
    ).thenAnswer((_) => Stream<RelayStatistics>.empty());
    when(
      mockHealthMonitor.queueStats,
    ).thenAnswer((_) => Stream<QueueSyncManagerStats>.empty());
    when(
      mockHealthMonitor.messageDeliveryStream,
    ).thenAnswer((_) => Stream<String>.empty());

    // Stub connection info stream & current info
    when(
      mockBleService.connectionInfo,
    ).thenAnswer((_) => StreamController<ConnectionInfo>.broadcast().stream);
    when(
      mockBleService.currentConnectionInfo,
    ).thenReturn(const ConnectionInfo(isConnected: false, isReady: false));
    when(
      mockBleService.receivedBinaryStream,
    ).thenAnswer((_) => StreamController<BinaryPayload>.broadcast().stream);
    when(
      mockBleService.identityRevealed,
  ...
  group('Send operations', () {
    ...
    test(
      'sendMeshMessage delegates to relay coordinator when not direct',
      () async {
        ...
        ).thenAnswer((_) async => MeshSendResult.relay('msg-1', 'next-hop-1'));
    ...
    test(
      'sendMeshMessage with high priority passes priority through',
      () async {
        await initializeService(nodeId: 'my-node');
        ...
        ).thenAnswer((_) async => MeshSendResult.relay('msg-2', 'next-hop-2'));
    ...
    test('retryBinaryMedia delegates to bleService', () async {
      when(
        mockBleService.retryBinaryMedia(
    ...
    test(
      'sendMeshMessage direct delivery when connected to recipient',
      () async {
        await initializeService(nodeId: 'my-node');
        ...
        when(
          mockQueueCoordinator.queueDirectMessage(
            chatId: anyNamed('chatId'),
  ...
  group('Queue management', () {
    ...
    test('retryMessage delegates to queue coordinator', () async {
      when(
        mockQueueCoordinator.retryMessage('msg-1'),
      ).thenAnswer((_) async => true);
    ...
    test('removeMessage delegates to queue coordinator', () async {
      when(
        mockQueueCoordinator.removeMessage('msg-2'),
      ).thenAnswer((_) async => true);
    ...
    test('setPriority delegates to queue coordinator', () async {
      ...
      final result = await service.setPriority('msg-3', MessagePriority.urgent);
    ...
    test('retryAllMessages delegates to queue coordinator', () async {
      when(mockQueueCoordinator.retryAllMessages()).thenAnswer((_) async => 5);
    ...
    test('getQueuedMessagesForChat returns coordinator results', () {
      when(
        mockQueueCoordinator.getQueuedMessagesForChat('chat-1'),
      ).thenReturn([]);

      final messages = service.getQueuedMessagesForChat('chat-1');

      expect(messages, isEmpty);
      verify(mockQueueCoordinator.getQueuedMessagesForChat('chat-1')).called(1);
    });
  });
  ...
  group('Statistics & streams', () {
    ...
    test('refreshMeshStatus calls broadcastMeshStatus on health monitor', () {
      ...
      verify(
        mockHealthMonitor.broadcastMeshStatus(
          isInitialized: anyNamed('isInitialized'),
          currentNodeId: anyNamed('currentNodeId'),
  ...
  group('Binary handling', () {
    ...
    test('retryBinaryMedia returns false on failure', () async {
      ...
      final result = await service.retryBinaryMedia(transferId: 'bad-transfer');
  ...
  group('Error handling', () {
    test(
      'sendMeshMessage catches exceptions and returns error result',
      () async {
    ...
    test(
      'initialize does not re-initialize when already initialized',
      () async {
