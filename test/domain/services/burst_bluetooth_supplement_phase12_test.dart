/// Phase 12 supplement: targeted branch coverage for BurstScanningController
...
void main() {
  ...
  group('BurstScanningController — supplement', () {
    ...
    test('startBurstScanning returns early when kill switch is active', () async {
      ...
      final killWarnings = logRecords.where(
        (r) =>
  ...
  group('BluetoothStateInfo — supplement', () {
    ...
    test('isReady reflects poweredOn correctly', () {
      final on = BluetoothStateInfo(
        state: BluetoothLowEnergyState.poweredOn,
        isReady: true,
        timestamp: DateTime.now(),
      );
      final off = BluetoothStateInfo(
