/// BluetoothStateMonitor extended tests.
///
...
void main() {
  ...
  group('BluetoothStateMonitor singleton extended', () {
    ...
    test('factory constructor returns same instance', () {
      expect(
        identical(BluetoothStateMonitor(), BluetoothStateMonitor.instance),
        isTrue,
      );
    });
    ...
    test('mixed unsupported and poweredOff resolves to poweredOff', () {
      expect(
        monitor.combineStatesForTesting(
    ...
    test('mixed unsupported and poweredOn resolves to poweredOn', () {
      expect(
        monitor.combineStatesForTesting(
    ...
    test(
      'runtime poweredOff state wins over transient unsupported callbacks',
      () {
        expect(
          monitor.combineObservedStatesForTesting(
            previousState: BluetoothLowEnergyState.poweredOff,
    ...
    test(
      'runtime unauthorized state wins over transient unsupported callbacks',
      () {
        expect(
          monitor.combineObservedStatesForTesting(
    ...
    test(
      'startup transient poweredOff to poweredOn within debounce stays warmingUp then becomes ready',
      () async {
        await monitor.simulateObservedStatesForTesting(
          central: BluetoothLowEnergyState.poweredOff,
          peripheral: BluetoothLowEnergyState.poweredOff,
          previousState: BluetoothLowEnergyState.unknown,
          isInitial: true,
        );

        expect(monitor.availabilityPhase, BluetoothAvailabilityPhase.warmingUp);
        expect(monitor.isBluetoothReady, isFalse);

        await Future<void>.delayed(const Duration(milliseconds: 500));

        await monitor.simulateObservedStatesForTesting(
          central: BluetoothLowEnergyState.poweredOn,
          peripheral: BluetoothLowEnergyState.poweredOn,
          previousState: BluetoothLowEnergyState.poweredOff,
        );

        expect(monitor.availabilityPhase, BluetoothAvailabilityPhase.ready);
        expect(monitor.isBluetoothReady, isTrue);

        await Future<void>.delayed(const Duration(milliseconds: 2100));

        expect(monitor.availabilityPhase, BluetoothAvailabilityPhase.ready);
        expect(monitor.currentState, BluetoothLowEnergyState.poweredOn);
      },
    );

    test(
      'runtime transient poweredOff to poweredOn within debounce does not emit disabled availability',
      () async {
        await monitor.simulateObservedStatesForTesting(
          central: BluetoothLowEnergyState.poweredOn,
          peripheral: BluetoothLowEnergyState.poweredOn,
        ...
        await monitor.simulateObservedStatesForTesting(
          central: BluetoothLowEnergyState.poweredOff,
          peripheral: BluetoothLowEnergyState.poweredOff,
          previousState: BluetoothLowEnergyState.poweredOn,
        );
    ...
    test('sustained runtime poweredOff beyond debounce becomes disabled', () async {
      await monitor.simulateObservedStatesForTesting(
        central: BluetoothLowEnergyState.poweredOn,
        peripheral: BluetoothLowEnergyState.poweredOn,
        previousState: BluetoothLowEnergyState.unknown,
      ...
      expect(monitor.availabilityPhase, BluetoothAvailabilityPhase.disabled);
      expect(monitor.currentState, BluetoothLowEnergyState.poweredOff);
      expect(monitor.isBluetoothReady, isFalse);
    });
  });
  ...
  group('BluetoothLowEnergyState values', () {
    test('includes all expected states', () {
      expect(
        BluetoothLowEnergyState.values,
        containsAll([
  ...
  group('BluetoothStatusMessage for every BluetoothMessageType', () {
    final factories =
        <BluetoothMessageType, BluetoothStatusMessage Function(String)>{
