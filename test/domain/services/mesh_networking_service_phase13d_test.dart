/// Phase 13d: Additional MeshNetworkingService coverage
...
void main() {
  ...
  group('Direct send paths', () {
    test('direct send with long messageId truncates for logging', () async {
      ...
      when(mockQueueCoordinator.queueDirectMessage(
        ...
        senderPublicKey: anyNamed('senderPublicKey'),
          ...
          (_) async => 'abcdefghijklmnopqrstuvwxyz1234567890');
    ...
    test('sendMeshMessage with short recipientPublicKey', () async {
      ...
      when(mockRelayCoordinator.sendRelayMessage(
        ...
        (_) async => MeshSendResult.relay('msg-r', 'next'),
      );
  ...
  group('Relay callbacks', () {
    test('handleRelayDecision processes decision with long messageId',
      ...
      final captured = verify(mockMessageHandler.initializeRelaySystem(
  ...
  group('Direct delivery checks', () {
    test('canDeliverDirectly false when connected but not ready', () async {
      ...
      when(mockBleService.currentConnectionInfo).thenReturn(
        const ConnectionInfo(isConnected: true, isReady: false),
      );
      ...
      when(mockRelayCoordinator.sendRelayMessage(
        ...
        (_) async => MeshSendResult.relay('msg-nd', 'next'),
      );
    ...
    test('canDeliverDirectly false when session ID does not match', () async {
      ...
      when(mockRelayCoordinator.sendRelayMessage(
        ...
        (_) async => MeshSendResult.relay('msg-mm', 'next'),
      );
  ...
  group('refreshMeshStatus', () {
    test('refreshMeshStatus broadcasts status with nodeId after init',
      ...
      final verification = verify(mockHealthMonitor.broadcastMeshStatus(
