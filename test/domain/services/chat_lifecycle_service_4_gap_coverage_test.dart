...
void main() {
  ...
  group('ChatLifecycleService – gap coverage', () {
    ...
    setUp(() async {
      ...
      archiveMgmt = ArchiveManagementService.withDependencies(
    ...
    group('getComprehensiveChatAnalytics', () {
      ...
      test(
        'gracefully handles when getChatAnalytics catches internal errors',
        () async {
          ...
          final throwingService = ChatLifecycleService(
            chatsRepository: _ThrowingChatsRepo(),
            messageRepository: messageRepo,
            archiveRepository: archiveRepo,
            archiveManagementService: archiveMgmt,
            cacheState: cacheState,
          ...
          final analytics = await throwingService.getComprehensiveChatAnalytics(
            'chat_1',
          );
    ...
    group('deleteMessages edge cases', () {
      ...
      test('deleting nonexistent IDs returns partial', () async {
        final result = await service.deleteMessages(
          messageIds: [MessageId('nope1'), MessageId('nope2')],
        );
    ...
    group('toggleChatArchive failure paths', () {
      test('enhanced archive failure returns failure result', () async {
        ...
        final result = await service.toggleChatArchive(
          'chat_1',
          useEnhancedArchive: true,
        );
