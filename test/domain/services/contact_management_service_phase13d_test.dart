/// Phase 13d: Supplementary tests for ContactManagementService
...
void main() {
  ...
  setUp(() {
    ...
    ContactManagementService.setInstance(
  ...
  group('singleton and factory constructors', () {
    ...
    test('fromServiceLocator throws when resolvers not configured', () {
      ...
      expect(
        () => ContactManagementService.fromServiceLocator(),
    ...
    test('fromServiceLocator works when resolvers are configured', () {
      ...
      ContactManagementService.configureDependencyResolvers(
        contactRepositoryResolver: () => contactRepo,
  ...
  group('getAllEnhancedContacts error path', () {
    test('returns empty list on exception', () async {
      ...
      final brokenService = ContactManagementService.withDependencies(
        contactRepository: brokenRepo,
        messageRepository: messageRepo,
      );
  ...
  group('createContactGroup error path', () {
    test('returns failure on internal error', () async {
      ...
      final result = await service.createContactGroup(
        name: 'Test Group',
      ...
      final dupResult = await service.createContactGroup(
        name: 'Test Group',
        description: 'Again',
      );
  ...
  group('searchContacts with filters', () {
    ...
    test('filter by onlyRecentlyActive', () async {
      ...
      expect(
        result.contacts.every((c) => c.isRecentlyActive),
        isTrue,
      );
    });
    ...
    test('filter by securityLevel', () async {
      ...
      expect(
        result.contacts.every((c) => c.securityLevel == SecurityLevel.low),
        isTrue,
      );
    });
  });
  ...
  group('searchContacts sort options', () {
    test('sort by name ascending', () async {
      ...
      if (result.contacts.length >= 2) {
        expect(
          result.contacts.first.displayName
    ...
    test('sort descending reverses order', () async {
      ...
      final asc = await service.searchContacts(
        query: '',
        filter: const ContactSearchFilter(),
        sortBy: ContactSortOption.name,
        ascending: true,
      );
      final desc = await service.searchContacts(
        query: '',
        filter: const ContactSearchFilter(),
        sortBy: ContactSortOption.name,
        ascending: false,
  ...
  group('deleteContactById with repository returning false', () {
    test('logs warning when deleteContact returns false', () async {
      ...
      final svc = ContactManagementService.withDependencies(
        contactRepository: repoNoDelete,
        messageRepository: messageRepo,
      );
    ...
    test('throws and catches when repository delete throws', () async {
      ...
      final svc = ContactManagementService.withDependencies(
        contactRepository: repoThrowOnDelete,
        messageRepository: messageRepo,
      );
  ...
  group('getContactAnalytics with data', () {
    test('computes distributions and oldest/newest', () async {
      ...
      expect(
          analytics.averageContactAge, isNot(equals(Duration.zero)));
    });
  });
  ...
  group('bulk delete contacts', () {
    test('deletes multiple contacts and reports results', () async {
      final result = await service.deleteContacts(
          ['pk-alice', 'pk-bob', 'nonexistent']);
