...
import 'group_messaging_service_phase12_test.mocks.dart';

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

Contact _contact({required String key, String name = 'User'}) => Contact(publicKey: key,
 displayName: name,
...
void main() {
 ...
 service = GroupMessagingService(groupRepo: groupRepo,
 ...
 when(contactRepo.getContact('member-1'))
 .thenAnswer((_) async => _contact(key: 'member-1', name: 'Alice'));
 when(messageQueue.queueMessage(chatId: anyNamed('chatId'),
 ...
 when(groupRepo.updateDeliveryStatus(any, any, any))
 .thenAnswer((_) async {});

 final msg = await service.sendGroupMessage(groupId: 'grp-1',
 ...
 verify(messageQueue.queueMessage(chatId: 'chat_member-1',
 content: 'hello',
 ...
 expect(() => service.sendGroupMessage(groupId: 'missing',
 senderKey: 'sender-key',
 content: 'hello',
),
 ...
 when(groupRepo.getGroup('grp-1')).thenAnswer((_) async => group);
 when(groupRepo.saveGroupMessage(any)).thenAnswer((_) async {});
 when(contactRepo.getContact('ghost-member'))
 ...
 await Future<void>.delayed(const Duration(milliseconds: 50));
 verify(groupRepo.updateDeliveryStatus(any,
 'ghost-member',
 MessageDeliveryStatus.failed,
)).called(1);
 verifyNever(messageQueue.queueMessage(chatId: anyNamed('chatId'),
 content: anyNamed('content'),
 ...
 final group = _group(memberKeys: ['sender-key', 'member-1', 'member-2', 'member-3'],
);
 when(groupRepo.getGroup('grp-1')).thenAnswer((_) async => group);
 when(groupRepo.saveGroupMessage(any)).thenAnswer((_) async {});
 when(contactRepo.getContact(any)).thenAnswer((inv) async =>
 _contact(key: inv.positionalArguments[0] as String),
);
 when(messageQueue.queueMessage(chatId: anyNamed('chatId'),
 content: anyNamed('content'),
 recipientPublicKey: anyNamed('recipientPublicKey'),
 ...
 expect(msg.deliveryStatus.containsKey('sender-key'), isFalse);
 await Future<void>.delayed(const Duration(milliseconds: 50));
 verify(messageQueue.queueMessage(chatId: anyNamed('chatId'),
 content: 'hey all',
 recipientPublicKey: anyNamed('recipientPublicKey'),
 senderPublicKey: 'sender-key',
 priority: MessagePriority.normal,
 ...
 await service.sendGroupMessage(groupId: 'grp-1',
 senderKey: 'sender-key',
 content: 'hello',
);
 ...
 final group = _group(memberKeys: ['sender-key', 'member-ok', 'member-gone', 'member-err'],
);
 when(groupRepo.getGroup('grp-1')).thenAnswer((_) async => group);
 when(groupRepo.saveGroupMessage(any)).thenAnswer((_) async {});
 when(groupRepo.updateDeliveryStatus(any, any, any))
 ...
 // member-ok: normal
 when(contactRepo.getContact('member-ok'))
 .thenAnswer((_) async => _contact(key: 'member-ok', name: 'OK'));
 // member-gone: not found
 when(contactRepo.getContact('member-gone'))
 .thenAnswer((_) async => null);
 // member-err: contact exists but queue throws
 when(contactRepo.getContact('member-err'))
 .thenAnswer((_) async => _contact(key: 'member-err', name: 'Err'));

 when(messageQueue.queueMessage(chatId: 'chat_member-ok',
 content: anyNamed('content'),
 recipientPublicKey: 'member-ok',
 senderPublicKey: anyNamed('senderPublicKey'),
 priority: anyNamed('priority'),
)).thenAnswer((_) async => 'q1');
 when(messageQueue.queueMessage(chatId: 'chat_member-err',
 content: anyNamed('content'),
 ...
 when(groupRepo.getGroupMessages('grp-1', limit: 10))
 ...
 when(groupRepo.getGroupMessages('grp-1', limit: 50))
 .thenAnswer((_) async => []);
 ...
 final gm = GroupMessage(id: 'gm-42',
 groupId: 'grp-1',
 senderKey: 'sender-key',
 content: 'test',
 timestamp: DateTime(2025, 1, 1),
 ...
 verify(groupRepo.updateDeliveryStatus('gm-1',
 'member-1',
 ...
 await service.markDeliveredForMember(const MessageId('gm-1'),
 ...
 verify(groupRepo.updateDeliveryStatus('gm-2',
 'member-2',
 MessageDeliveryStatus.failed,
)).called(1);
 });
 ...
 await service.markFailedForMember(const MessageId('gm-2'),
 ...
 final summary =
 await service.getDeliverySummary(const MessageId('gm-s'));
 ...
 final summary =
 await service.getDeliverySummary(const MessageId('no-msg'));
 ...
 verify(groupRepo.updateDeliveryStatus('gm-err',
 'member-x',
 MessageDeliveryStatus.delivered,
)).called(1);
 });
