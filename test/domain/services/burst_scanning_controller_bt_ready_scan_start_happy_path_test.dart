...
void main() {
 ...
 BluetoothStateMonitor.overrideCurrentState(BluetoothLowEnergyState.unknown,
);
 BluetoothStateMonitor().dispose();
 KillSwitches.disableDiscoveryScheduler = false;
 });
 ...
 expect(logRecords.any((r) => r.message.contains('Scan started successfully')),
 isTrue,
);
 ...
 final (controller, ble) = await initReady(canAcceptMore: false,
 activeCount: 1,
 maxConns: 1,
);
 fakeAsync((async) {
 ...
 expect(logRecords.any((r) => r.message.contains('already at max connections'),
),
 isTrue,
);
 ...
 expect(controller.getCurrentStatus().isBurstActive, isFalse);
 expect(logRecords.any((r) => r.message.contains('Failed to start scanning'),
),
 isTrue,
);
 ...
 expect(controller.getCurrentStatus().isBurstActive, isFalse);
 expect(logRecords.any((r) => r.message.contains('Scan stopped successfully'),
),
 isTrue,
);
 ...
 expect(controller.getCurrentStatus().isBurstActive, isFalse);
 expect(logRecords.any((r) => r.message.contains('Error stopping scan'),
),
 isTrue,
);
 ...
 expect(logRecords.any((r) => r.message.contains('connection health check'),
),
 isTrue,
);
 ...
 expect(logRecords.any((r) => r.message.contains('Power stats updated'),
),
 isTrue,
);
 ...
 // Shorten burst
 unawaited(controller.triggerManualScan(delay: const Duration(milliseconds: 100),
));
 async.flushMicrotasks();
 ...
 final (controller, ble) = await initReady();
 fakeAsync((async) {
 unawaited(controller.triggerManualScan(delay: const Duration(milliseconds: 50),
));
 async.flushMicrotasks();
 ...
 final (controller, _) = await initReady();
 controller.reportConnectionSuccess(rssi: -55,
 connectionTime: 200,
 dataTransferSuccess: true,
);
 expect(controller.getCurrentStatus().powerStats.qualityMeasurementsCount,
 ...
 expect(controller.getCurrentStatus().powerStats.consecutiveFailedChecks,
 greaterThanOrEqualTo(1),
);
 controller.dispose();
 });
 ...
 expect(ble.startCalls, 0);
 expect(logRecords.any((r) => r.message.contains('Bluetooth not ready')),
 isTrue,
);
