/// Supplementary tests for ArchiveSearchService
...
class _FakeArchiveRepository implements IArchiveRepository {
 ...
 return ArchiveSearchResult.fromResults(messages: const [],
 chats: const [],
 query: query,
 filter: filter,
...
void main() {
 ...
 service = ArchiveSearchService.withDependencies(archiveRepository: repository,
);
 ...
 final result = await service.fuzzySearch(query: 'test',
 ...
 final result = await service.fuzzySearch(query: 'hello',
 filter: filter,
);
 expect(result, isNotNull);
 });
 ...
 final result = await service.searchByDateRange(query: 'test',
 startDate: start,
 endDate: end,
);
 expect(result.query, isNotEmpty);
 ...
 final result = await service.searchByDateRange(query: 'recent',
 startDate: start,
 endDate: end,
 ...
 test('returns error result when repository throws', () async {
 repository.throwOnSearch = true;
 final result = await service.searchByDateRange(query: 'fail',
 ...
 group('dispose', () {
 test('completes without error', () async {
 final disposable = ArchiveSearchService.withDependencies(archiveRepository: repository,
);
