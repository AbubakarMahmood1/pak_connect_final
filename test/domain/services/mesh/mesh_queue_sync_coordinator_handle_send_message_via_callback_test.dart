...
class _FakeConnectionService extends Fake implements IConnectionService {
 ...
 @override
 Future<bool> sendPeripheralMessage(String content, {
 String? messageId,
 }) async {
 peripheralSendCount++;
 return sendResult;
 ...
 @override
 void registerQueueSyncHandler(Future<bool> Function(QueueSyncMessage message, String fromNodeId) handler,
) {
 _syncHandler = handler;
 }

 Future<bool> invokeSyncHandler(QueueSyncMessage msg,
 String fromNodeId,
) async {
...
class _CallbackCapturingQueue extends Fake
 ...
 @override
 QueueStatistics getStatistics() => QueueStatistics(totalQueued: messages.length,
 totalDelivered: 0,
 totalFailed: 0,
 ...
 final msg = QueuedMessage(id: 'q-${messages.length}',
 chatId: chatId,
 content: content,
 ...
 @override
 QueueSyncMessage createSyncMessage(String nodeId) => QueueSyncMessage(nodeId: nodeId,
 queueHash: 'hash-fake',
...
class _FakeSyncManager extends Fake implements QueueSyncManagerContract {
 ...
 QueueSyncResult syncResponseResult = QueueSyncResult.success(messagesReceived: 0,
 messagesUpdated: 0,
 messagesSkipped: 0,
 finalHash: 'hash',
 ...
 @override
 QueueSyncManagerStats getStats() => const QueueSyncManagerStats(totalSyncRequests: 0,
 successfulSyncs: 0,
 failedSyncs: 0,
 ...
 @override
 Future<QueueSyncResponse> handleSyncRequest(QueueSyncMessage syncMessage,
 String fromNodeId,
) async => syncRequestResponse;

 @override
 Future<QueueSyncResult> processSyncResponse(QueueSyncMessage responseMessage,
...
void main() {
 ...
 coordinator = MeshQueueSyncCoordinator(bleService: bleService,
 ...
 await coordinator.initialize(nodeId: nodeId,
 messageQueue: queue,
 ...
 expect(queue.getMessageById('msg-no-conn')?.status,
 ...
 test('does not fail when connected peer does not match recipient',
 () async {
 ...
 queue.addTestMessage(_testMessage(id: 'msg-mismatch', recipientPublicKey: 'intended-peer'),
);
 ...
 expect(queue.getMessageById('msg-mismatch')?.status,
 QueuedMessageStatus.pending,
);
 },
);
 ...
 queue.addTestMessage(_testMessage(id: 'msg-relay', recipientPublicKey: 'final-recipient'),
);
 ...
 expect(queue.getMessageById('msg-relay')?.status,
 QueuedMessageStatus.pending,
);
 });
 ...
 queue.addTestMessage(_testMessage(id: 'msg-match', recipientPublicKey: 'intended-peer'),
);
 ...
 queue.addTestMessage(_testMessage(id: 'msg-eph', recipientPublicKey: 'intended-peer'),
);
 ...
 queue.addTestMessage(_testMessage(id: 'msg-pkey', recipientPublicKey: 'intended-peer'),
);
 ...
 queue.addTestMessage(_testMessage(id: 'pending-1', recipientPublicKey: 'recipient-1'),
);
 await initCoordinator();
 coordinator.startConnectionMonitoring();

 bleService.emitConnection(const ConnectionInfo(isConnected: true,
 isReady: true,
 ...
 bleService.emitConnection(const ConnectionInfo(isConnected: false, isReady: false),
);
 await Future.delayed(const Duration(milliseconds: 50));
 ...
 final highPri = _testMessage(id: 'high-1',
 recipientPublicKey: 'recipient-1',
 priority: MessagePriority.high,
 queuedAt: DateTime(2024, 1, 2),
);
 final normalPri = _testMessage(id: 'normal-1',
 recipientPublicKey: 'recipient-1',
 ...
 queue.addTestMessage(_testMessage(id: 'other-1', recipientPublicKey: 'other-device'),
);
 await initCoordinator();
 coordinator.startConnectionMonitoring();
 ...
 final responseMsg = QueueSyncMessage(nodeId: 'node-1',
 queueHash: 'h1',
 messageIds: [],
 syncTimestamp: DateTime.now(),
 syncType: QueueSyncType.response,
);
 syncManager.syncRequestResponse = QueueSyncResponse.success(responseMessage: responseMsg,
 missingMessages: [],
 excessMessages: [],
);
 await initCoordinator();
 ...
 final syncReq = QueueSyncMessage(nodeId: 'peer-1',
 queueHash: 'h2',
 messageIds: ['m1'],
 syncTimestamp: DateTime.now(),
 syncType: QueueSyncType.request,
 ...
 final syncResp = QueueSyncMessage(nodeId: 'peer-2',
 queueHash: 'h3',
 messageIds: ['m2'],
 syncTimestamp: DateTime.now(),
 syncType: QueueSyncType.response,
 ...
 final syncReq = QueueSyncMessage(nodeId: 'peer-3',
 queueHash: 'h4',
 messageIds: [],
 syncTimestamp: DateTime.now(),
 syncType: QueueSyncType.request,
 ...
 queue.capturedOnStatsUpdated?.call(QueueStatistics(totalQueued: 1,
 totalDelivered: 0,
 totalFailed: 0,
 pendingMessages: 1,
 sendingMessages: 0,
