...
void main() {
 ...
 NetworkTopology topo(Map<String, Set<String>> connections,
 Map<String, ConnectionQuality> qualities,
) {
 return NetworkTopology(connections: connections,
 ...
 final topology = topo({'A': {'B'}, 'B': {'A'}},
 {qk('A', 'B'): entry.key},
);

 final routes = await calculator.calculateRoutes(from: 'A',
 to: 'B',
 availableHops: ['B'],
 topology: topology,
);
 ...
 final r1 = await calculator.calculateRoutes(from: 'A',
 to: 'B',
 availableHops: ['B'],
 topology: topology,
);
 final r2 = await calculator.calculateRoutes(from: 'A',
 to: 'B',
 availableHops: ['B'],
 topology: topology,
);
 ...
 final topology = topo({'X': {'Y'}, 'Y': {'X'}},
 {qk('X', 'Y'): ConnectionQuality.good},
);

 await calculator.calculateRoutes(from: 'X',
 to: 'Y',
 availableHops: ['Y'],
 topology: topology,
);
 ...
 final routes = await calculator.calculateRoutes(from: 'S',
 to: 'D',
 availableHops: ['R'],
 topology: topology,
 maxHops: 2,
 ...
 final routes = await calculator.calculateRoutes(from: 'S2',
 to: 'D2',
 availableHops: ['R2'],
 topology: topology,
 maxHops: 2,
 ...
 final routes = await calculator.calculateRoutes(from: 'src',
 to: 'dst',
 ...
 for (int i = 0; i < routes.length - 1; i++) {
 expect(routes[i].estimatedLatency,
