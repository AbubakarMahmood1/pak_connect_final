...
/// Phase 13.2: Supplementary tests for network_topology_analyzer.dart
...
void main() {
  ...
  group('NetworkTopologyAnalyzer — discoverNodes', () {
    test('adds connection when BLE is connected and ready', () async {
      final bleService = _FakeConnectionService(
        publicKey: 'my-node-id-123456789',
  ...
  group('NetworkTopologyAnalyzer — estimateConnectionQuality', () {
    test('connected & ready yields good quality', () async {
      ...
      final quality = topology.getConnectionQuality(
        'nodeA-123456789',
        'nodeB-987654321',
      );
  ...
  group('NetworkTopologyStats', () {
    test('toJson() returns correct map', () {
      ...
      final stats = NetworkTopologyStats(
        totalNodes: 5,
        totalConnections: 7,
        averageQuality: 0.85,
        isConnected: true,
  ...
  group('NetworkTopologyAnalyzer — updateConnectionQuality', () {
    test('does not notify if quality stays the same', () async {
      final goodMetrics = ConnectionMetrics(
        signalStrength: 0.7,
        latency: 100.0,
        packetLoss: 0.05,
        throughput: 0.8,
    ...
    test('updates when quality degrades to poor', () async {
      ...
      final poorMetrics = ConnectionMetrics(
        signalStrength: 0.2,
        latency: 3000.0,
        packetLoss: 0.6,
        throughput: 0.1,
      ...
      final q = topology.getConnectionQuality('A', 'B');
      expect(
        q == ConnectionQuality.poor || q == ConnectionQuality.unreliable,
        true,
      );
    });
  });
  ...
  group('NetworkTopologyAnalyzer — connection key symmetry', () {
    test('updateConnectionQuality is symmetric for A→B and B→A', () async {
      ...
      final q1 = analyzer.getNetworkTopology().getConnectionQuality(
        'NodeA', 'NodeB',
      );

      // Update B→A (same key due to sorting)
      await analyzer.updateConnectionQuality('NodeB', 'NodeA', metrics);
      final q2 = analyzer.getNetworkTopology().getConnectionQuality(
        'NodeB', 'NodeA',
      );
