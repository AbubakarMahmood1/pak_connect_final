...
String _reassembleText(List<MessageChunk> chunks) {
  final reassembler = MessageReassembler();
  String? result;
  for (final chunk in chunks) {
    result = reassembler.addChunk(MessageChunk.fromBytes(chunk.toBytes()));
  }
  return result!;
}
...
void main() {
  ...
  group('MessageChunk', () {
    ...
    test(
      'fromBytes throws FormatException on invalid format (too few parts)',
      () {
    ...
    test('fromBytes preserves pipes in legacy text content', () {
      final raw = utf8.encode('id|0|1|0|extra|field');

      final chunk = MessageChunk.fromBytes(Uint8List.fromList(raw));

      expect(chunk.content, 'extra|field');
    });
  });
  ...
  group('MessageFragmenter', () {
    ...
    test('fragment single chunk for small message', () {
      ...
      expect(_reassembleText(chunks), equals('Hi'));
    });
    ...
    test('fragment multiple chunks for large message', () {
      ...
      expect(_reassembleText(chunks), equals(longMsg));
    });

    test(
      'fragment respects maxChunkSize (each chunk.toBytes().length <= maxChunkSize)',
      () {
        final msg = 'B' * 300;
        const mtu = 60;
    ...
    test('fragment preserves original message content across all chunks', () {
      ...
      expect(_reassembleText(chunks), equals(msg));
    });

    test('fragment round-trips Unicode and pipes within the MTU', () {
      const mtu = 55;
      final message = 'اردو | emoji 🧭 | 漢字 ' * 12;

      final chunks = MessageFragmenter.fragment(message, mtu);

      expect(_reassembleText(chunks), message);
      for (final chunk in chunks) {
        expect(chunk.toBytes().length + 5, lessThanOrEqualTo(mtu));
      }
    });
    ...
    test('fragmentBytes keeps four-digit chunk headers within the MTU', () {
      const mtu = 40;
      final data = Uint8List.fromList(List.generate(12000, (i) => i % 256));

      final chunks = MessageFragmenter.fragmentBytes(data, mtu, 'large');

      expect(chunks.length, greaterThan(999));
      for (final chunk in chunks) {
        expect(chunk.toBytes().length + 5, lessThanOrEqualTo(mtu));
      }
      final reassembler = MessageReassembler();
    ...
    test('fragmentBytesWithId delegates to fragmentBytes', () {
      ...
      final chunksA = MessageFragmenter.fragmentBytesWithId(data, 200, mid);
      final chunksB = MessageFragmenter.fragmentBytes(data, 200, mid.value);
    ...
    test(
      'fragmentBytes assigns a generated wire id, not a slice of the caller id',
      () {
        final data = Uint8List.fromList([1, 2, 3]);
    ...
    test('fragmentBytes assigns distinct wire ids even when caller ids share a '
      ...
      expect(
        chunks1.first.messageId,
        isNot(equals(chunks2.first.messageId)),
        reason: 'colliding caller ids must not produce colliding wire ids',
      );
    });

    test(
      'interleaved multi-chunk messages with colliding caller ids reassemble '
      'independently (PC-FRAG-001)',
      () {
        final data1 = Uint8List.fromList(List.generate(300, (i) => i % 256));
        final data2 = Uint8List.fromList(
          List.generate(300, (i) => (255 - (i % 256))),
        );
        ...
        for (var i = 0; i < maxLen; i++) {
          if (i < chunks1.length) {
            final got = reassembler.addChunkBytes(
              MessageChunk.fromBytes(chunks1[i].toBytes()),
            );
            if (got != null) r1 = got;
          }
          if (i < chunks2.length) {
            final got = reassembler.addChunkBytes(
              MessageChunk.fromBytes(chunks2[i].toBytes()),
            );
            if (got != null) r2 = got;
          }
        }
  ...
  group('MessageReassembler', () {
    ...
    test(
      'addChunkBytes handles binary chunks (base64 decode + concatenate)',
      () {
    ...
    test(
      ...
      () {
        ...
        final original = Uint8List.fromList(List.generate(300, (i) => i % 256));
        final chunks = MessageFragmenter.fragmentBytes(
          original,
          80,
          'roundtrip123456',
