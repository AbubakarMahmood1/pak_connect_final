// Supplementary tests for _ArchiveRepositoryMappingHelper
...
void main() {
  ...
  setUpAll(() async {
    await TestSetup.initializeTestEnvironment(dbLabel: 'mapping_helper_p13');
  ...
  Future<ArchiveId> insertArchiveRow(
    ...
    await db.insert('archived_chats', {
      ...
      'contact_public_key': contactPublicKey == '_DEFAULT_PK_'
          ? 'pk_$chatId'
          : contactPublicKey,
      'archived_at': now,
  ...
  group('mapToArchivedMessage — null handling', () {
    ...
    test(
      'legacy-prefixed metadata_json is ignored instead of crashing',
      () async {
        final repo = ArchiveRepository();
        await repo.initialize();

        final archiveId = await insertArchiveRow('chat8b', 'Hank Legacy');
        await insertMessageRow(
          archiveId,
          'msg8b',
          'chat8b',
          metadataJson: '${ArchiveCrypto.unsupportedLegacyPrefix}ciphertext',
        );

        final chat = await repo.getArchivedChat(archiveId);
        expect(chat, isNotNull);
        expect(chat!.messages, hasLength(1));
        expect(chat.messages.first.metadata, isNull);
      },
    );
    ...
    test(
      'legacy-prefixed reactions_json is ignored instead of crashing',
      () async {
        final repo = ArchiveRepository();
        await repo.initialize();

        final archiveId = await insertArchiveRow('chat10b', 'Jack Legacy');
        await insertMessageRow(
          archiveId,
          'msg10b',
          'chat10b',
          reactionsJson: '${ArchiveCrypto.unsupportedLegacyPrefix}ciphertext',
        );

        final chat = await repo.getArchivedChat(archiveId);
        expect(chat, isNotNull);
        expect(chat!.messages.first.reactions, isEmpty);
      },
    );
    ...
    test('archive_metadata_json fallback to default when null', () async {
      ...
      expect(msg.archiveMetadata.indexingStatus, ArchiveIndexingStatus.indexed);
  ...
  group('mapToArchivedChatSummary — edge cases', () {
    test('last_message_time null yields null in summary', () async {
      ...
      final summary = summaries.firstWhere((s) => s.contactName == 'Sam');
    ...
    test('estimated_size defaults to 0 when null', () async {
      ...
      final summaries = await repo.getArchivedChats();
      final summary = summaries.firstWhere((s) => s.contactName == 'Tina');
    ...
    test('is_compressed=1 maps to true', () async {
      ...
      final summaries = await repo.getArchivedChats();
      final summary = summaries.firstWhere((s) => s.contactName == 'Uma');
    ...
    test('tags default to empty and isSearchable defaults to true', () async {
      ...
      final summaries = await repo.getArchivedChats();
      final summary = summaries.firstWhere((s) => s.contactName == 'Vera');
    ...
    test('last_message_time set yields DateTime in summary', () async {
      ...
      final summaries = await repo.getArchivedChats();
      final summary = summaries.firstWhere((s) => s.contactName == 'Wade');
  ...
  group('mapToArchivedChat — metadata handling', () {
    ...
    test('legacy-prefixed archive metadata falls back to defaults', () async {
      final repo = ArchiveRepository();
      await repo.initialize();
      ...
      final archiveId = await insertArchiveRow(
        ...
        'Yolanda Legacy',
        metadataJson: '${ArchiveCrypto.unsupportedLegacyPrefix}ciphertext',
        archiveReason: 'legacy import',
      );
      await insertMessageRow(archiveId, 'msgM2b', 'chatM2b');

      final chat = await repo.getArchivedChat(archiveId);
      expect(chat, isNotNull);
      expect(chat!.metadata.version, '1.0');
      expect(chat.metadata.reason, 'legacy import');
    });

    test(
      'null archive_reason falls back to Unknown in default metadata',
      () async {
        final repo = ArchiveRepository();
        await repo.initialize();
    ...
    test('malformed custom_data_json is ignored instead of crashing', () async {
      final repo = ArchiveRepository();
      await repo.initialize();
      ...
      await insertMessageRow(archiveId, 'msgM6b', 'chatM6b');

      final chat = await repo.getArchivedChat(archiveId);
      expect(chat, isNotNull);
      expect(chat!.customData, isNull);
    });
  ...
  group('ArchiveCrypto — field encrypt/decrypt', () {
    ...
    test('detects unsupported legacy archive prefix', () {
      expect(
        ArchiveCrypto.isUnsupportedLegacyCiphertext(
          '${ArchiveCrypto.unsupportedLegacyPrefix}ciphertext',
        ),
        isTrue,
      );
      expect(
        ArchiveCrypto.isUnsupportedLegacyCiphertext('plain text'),
        isFalse,
      );
    });
  });
