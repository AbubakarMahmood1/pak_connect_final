...
/// Supplementary tests for ArchiveRepository
...
void main() {
 ...
 setUpAll(() async {
 await TestSetup.initializeTestEnvironment(dbLabel: 'archive_repo_p13',
);
 ...
 Future<ArchiveId> createArchiveDirectly(String chatId,
 String contactName,
 int messageCount, {
 int estimatedSize = 0,
 ...
 Future<void> createChatWithMessages(String chatId,
 String contactName,
 int messageCount,
) async {
 final db = await DatabaseHelper.database;
 ...
 for (int i = 0; i < messageCount; i++) {
 final message = Message(id: MessageId('msg_${chatId}_$i'),
 ...
 final result = await repo.archiveChat(chatId: 'chat_arc',
 ...
 final result = await repo.archiveChat(chatId: 'chat_nocomp',
 ...
 expect(result.warnings.any((w) => w.contains('Large archive')),
 false,
);
 });
 });
 ...
 final result = await repo.searchArchives(query: 'Test message',
 filter: const ArchiveSearchFilter(messageTypeFilter: ArchiveMessageTypeFilter(wasStarred: true),
),
);
 ...
 final result = await repo.searchArchives(query: 'Test message',
 filter: const ArchiveSearchFilter(messageTypeFilter: ArchiveMessageTypeFilter(isFromMe: true),
),
);
 ...
 await createArchiveDirectly('ch_st2',
 'Bob',
 5,
 isCompressed: true,
 ...
 final result = await repo.permanentlyDeleteArchive(ArchiveId('nonexistent'),
);
 ...
 final filter = ArchiveSearchFilter(dateRange: ArchiveDateRange(start: DateTime(2025, 1, 1),
