// Supplementary tests for ProtocolMessageHandler
...
void main() {
 ...
 final msg = ProtocolMessage.contactRequest(publicKey: 'pk_alice',
 displayName: 'Alice',
);

 final result = await handler.processProtocolMessage(message: msg,
 ...
 final msg = ProtocolMessage.contactRequest(publicKey: 'pk_bob',
 displayName: 'Bob',
);
 ...
 final msg = ProtocolMessage.contactAccept(publicKey: 'pk_carol',
 displayName: 'Carol',
);
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.contactAccept,
 payload: {},
 timestamp: DateTime.now(),
);

 await handler.processProtocolMessage(message: msg,
 fromDeviceId: 'device1',
 fromNodeId: 'node1',
);
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.cryptoVerification,
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.cryptoVerificationResponse,
 payload: {
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.friendReveal,
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.ping,
 payload: {},
 timestamp: DateTime.now(),
);
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.relayAck,
 payload: {},
 timestamp: DateTime.now(),
);
 ...
 final result = await handler.processCompleteProtocolMessage(content: content,
 fromDeviceId: 'device1',
 fromNodeId: 'node1',
 messageData: null,
);
 ...
 final result = await handler.processCompleteProtocolMessage(content: 'not valid protocol data',
 fromDeviceId: 'device1',
 fromNodeId: 'node1',
 messageData: null,
);
 ...
 final msg = ProtocolMessage.contactRequest(publicKey: 'pk_direct',
 displayName: 'Direct',
);

 final result = await handler.handleDirectProtocolMessage(message: msg,
 fromDeviceId: 'device1',
);
 ...
 final result = await handler.handleDirectProtocolMessage(message: badMsg,
 fromDeviceId: 'device1',
);
 ...
 final queueMsg = QueueSyncMessage(queueHash: 'hash-123',
 messageIds: const ['m1'],
 ...
 final queueMsg = QueueSyncMessage(queueHash: 'hash-456',
 messageIds: const ['m2'],
 syncTimestamp: DateTime.now(),
 nodeId: 'node-2',
 syncType: QueueSyncType.request,
 ...
 final msg = ProtocolMessage(type: ProtocolMessageType.queueSync,
 ...
 ProtocolMessageHandler.configureIdentityManagerResolver(() => _FakeIdentityManager(persistentId: 'my-persistent-key'),
);
 handler = ProtocolMessageHandler(securityService: securityService,
 requireV2Signature: false,
);
 ...
 final result = await handler.resolveMessageIdentities(encryptionSenderKey: 'enc-key',
 ...
 final result = await handler.resolveMessageIdentities(encryptionSenderKey: null,
 meshSenderKey: 'mesh-key',
 ...
 final result = await handler.resolveMessageIdentities(encryptionSenderKey: 'same-key',
 ...
 final result = await handler.getMessageEncryptionMethod(senderKey: 'sender',
 ...
 final result = await handler.checkQRIntroductionMatch(receivedHash: 'abc123',
 expectedHash: 'abc123',
);
 expect(result, isTrue);
 });
 ...
 await handler.handleQRIntroductionClaim(claimJson: '{"test": true}',
 fromDeviceId: 'device1',
);
 // No error expected
 });
 ...
 expect(logRecords.any((r) => r.message.contains('Current node ID set')),
 isTrue,
);
 });
 });
...
class _FakeSecurityService implements ISecurityService {
 ...
 @override
 Future<SecurityLevel> getCurrentLevel(String publicKey, [
 IContactRepository? repo,
]) async => SecurityLevel.low;

 @override
 Future<EncryptionMethod> getEncryptionMethod(String publicKey,
 IContactRepository repo,
 ...
 @override
 Future<String> encryptMessageByType(String message,
 String publicKey,
 IContactRepository repo,
 EncryptionType type,
) async => message;
 ...
 @override
 Future<String> decryptMessageByType(String encryptedMessage,
 String publicKey,
 IContactRepository repo,
 EncryptionType type,
 ...
 @override
 Future<Uint8List> encryptBinaryPayload(Uint8List data,
 String publicKey,
 IContactRepository repo,
) async => data;

 @override
 Future<Uint8List> decryptBinaryPayload(Uint8List data,
 String publicKey,
 IContactRepository repo,
) async => data;
