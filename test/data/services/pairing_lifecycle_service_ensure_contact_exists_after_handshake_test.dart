// PairingLifecycleService coverage
...
class _FakeContactRepository extends Fake implements ContactRepository {
 ...
 @override
 Future<void> saveContactWithSecurity(String publicKey,
 String displayName,
 ...
 contacts[publicKey] = Contact(publicKey: publicKey,
 ...
 @override
 Future<void> updateContactEphemeralId(String publicKey,
 String ephemeralId,
) async {
...
void main() {
 ...
 service = PairingLifecycleService(logger: Logger('Test'),
 ...
 await service.ensureContactExistsAfterHandshake('pk-123',
 'Alice',
 ephemeralId: 'eph-456',
);
 ...
 expect(contactRepo.contacts['pk-123']?.securityLevel,
 SecurityLevel.low,
);
 });
 ...
 contactRepo.contacts['pk-123'] = Contact(publicKey: 'pk-123',
 displayName: 'Alice',
 trustStatus: TrustStatus.newContact,
 ...
 await service.cacheSharedSecret(contactId: 'contact-pk',
 ...
 await service.cacheSharedSecret(contactId: '',
 sharedSecret: 'secret',
);
 ...
 await service.cacheSharedSecret(contactId: 'same-pk',
 ...
 contactRepo.contacts['eph-id'] = Contact(publicKey: 'eph-id',
 displayName: 'Bob',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
 firstSeen: DateTime.now(),
 ...
 await service.upgradeContactToMediumSecurity(theirEphemeralId: 'eph-id',
 ...
 expect(contactRepo.contacts['eph-id']?.securityLevel,
 SecurityLevel.medium,
);

 // Identity mapping registered
 expect(securityService.registeredMappings, hasLength(1));
 expect(securityService.registeredMappings[0],
 ...
 await service.upgradeContactToMediumSecurity(theirEphemeralId: null,
 ...
 await service.upgradeContactToMediumSecurity(theirEphemeralId: '',
 theirPersistentKey: 'pk',
);
 ...
 await service.upgradeContactToMediumSecurity(theirEphemeralId: 'unknown-eph',
 theirPersistentKey: 'pk',
);
 ...
 await service.handlePersistentKeyExchange(theirPersistentKey: 'pk',
);
 ...
 try {
 await service.handlePersistentKeyExchange(theirPersistentKey: 'their-persistent-key',
 displayName: 'Charlie',
);
 } catch (_) {
