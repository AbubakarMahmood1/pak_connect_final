...
class _ConfigurableRelayEngine implements domain_messaging.MeshRelayEngine {
  ...
  _ConfigurableRelayEngine({RelayStatistics? stats, this.seenMessageStore})
    : stats =
          stats ??
  ...
  Future<RelayProcessingResult> processIncomingRelay({
    ...
    final nextHop = availableNextHops.isNotEmpty
        ...
        : 'next-hop';
    capturedOnRelayMessage?.call(relayMessage, nextHop);
...
class _FakeMeshRelayEngineFactory implements IMeshRelayEngineFactory {
  ...
  domain_messaging.MeshRelayEngine create({
    ...
    lastEngine = _ConfigurableRelayEngine(seenMessageStore: seenMessageStore);
    return lastEngine!;
  }
}
...
MeshRelayMessage _buildRelayMessage({
  ...
  final effectiveRelayPayload = relayPayload ?? _buildInnerProtocolPayload();
  return MeshRelayMessage.createRelay(
    originalMessageId: originalMessageId,
    originalContent: '',
    metadata: RelayMetadata.create(
      originalMessageContent: effectiveRelayPayload,
    ...
    encryptedPayload: effectiveRelayPayload,
    originalMessageType: ProtocolMessageType.textMessage,
  );
}
...
void main() {
  ...
  group('RelayCoordinator — ', () {
    ...
    Future<void> initializeCoordinator([String nodeId = 'relay-node']) =>
        coordinator.initializeRelaySystem(currentNodeId: nodeId);
    ...
    group('handleMeshRelay', () {
      test(
        ...
        () async {
          await initializeCoordinator('my-node');
          final relayPayload = _buildInnerProtocolPayload(
            messageId: 'msg-self-inner',
            content: 'Hello self',
          );
          String? receivedId;
          ...
          final result = await coordinator.handleMeshRelay(
            ...
            messageData: {'innerProtocolMessage': relayPayload},
            currentHopCount: 0,
          );

          expect(result, isTrue);
          expect(receivedId, 'msg-self');
          expect(receivedContent, relayPayload);
        },
      );
      ...
      test(
        ...
        () async {
          await initializeCoordinator('node-x');
          final relayPayload = _buildInnerProtocolPayload(
            messageId: 'broadcast-inner',
      ...
      test(
        'relays to next hop when intendedRecipient is another node',
        () async {
          await initializeCoordinator('node-A');
          coordinator.setNextHopsProvider(() => ['node-C']);
          final relayPayload = _buildInnerProtocolPayload(
            messageId: 'msg-other-inner',
            content: 'msg for B',
          );
          var selfDelivered = false;
          String? nextHop;
          ...
          coordinator.onSendRelayMessage((_, hop) => nextHop = hop);
          ...
          expect(result, isTrue);
          expect(selfDelivered, isFalse);
          expect(nextHop, 'node-C');
        },
      );
      ...
      test('rejects plaintext relay payloads', () async {
        await initializeCoordinator();

        final result = await coordinator.handleMeshRelay(
          originalMessageId: 'plaintext',
    ...
    group('createOutgoingRelay', () {
      test(
        ...
        () async {
          await initializeCoordinator('my-relay');
          final relayPayload = _buildInnerProtocolPayload(
            messageId: 'orig-1-inner',
            content: 'payload',
          );
          ...
          expect(msg.originalContent, isEmpty);
          expect(msg.relayPayload, relayPayload);
        },
      );

      test('creates relay with specified recipient', () async {
        await initializeCoordinator('relay-node');
        final relayPayload = _buildInnerProtocolPayload(
          messageId: 'orig-2-inner',
          content: 'data',
        );
        final msg = await coordinator.createOutgoingRelay(
      ...
      test('returns null when relay engine is not initialized', () async {
        final msg = await coordinator.createOutgoingRelay(
          originalMessageId: 'no-node',
          content: _buildInnerProtocolPayload(messageId: 'no-node-inner'),
          originalSender: 's',
          intendedRecipient: 'r',
          currentHopCount: 0,
        );
      ...
      test('rejects invalid relay payloads', () async {
        await initializeCoordinator('relay-node');

        final msg = await coordinator.createOutgoingRelay(
          originalMessageId: 'invalid-payload',
          content: 'plaintext',
          originalSender: 'sender',
          intendedRecipient: 'target',
          currentHopCount: 0,
    ...
    group('handleRelayToNextHop', () {
      test('sends relay message via callback', () async {
        ...
        final relayPayload = _buildInnerProtocolPayload(
          messageId: 'r-msg-1-inner',
          content: 'hello',
        );
        final relayMsg = _buildRelayMessage(
        ...
        expect(
          captured!.meshRelayOriginalPayload?['innerProtocolMessage'],
          relayPayload,
        );
        expect(captured!.meshRelayOriginalPayload?['content'], isNull);
      });
      ...
      test('registers ACK timeout timer', () async {
        ...
        final relayMsg = _buildRelayMessage(
          ...
          originalSender: 's',
          finalRecipient: 'r',
          relayPayload: _buildInnerProtocolPayload(
            messageId: 'ack-timeout-inner',
            content: 'c',
          ),
        );
