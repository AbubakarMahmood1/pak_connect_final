...
void main() {
  ...
  group('friendReveal dispatch', () {
    test('does not emit an unverified display name', () async {
      String? capturedName;
    ...
    test('does not emit an unverified claimed persistent key', () async {
      String? capturedName;
      handler.onIdentityRevealed((name) => capturedName = name);
  ...
  group('queueSync dispatch', () {
    test('processes valid queueSync and triggers ACK', () async {
      ...
      String? capturedDeviceAddress;
      handler.onSendAckMessage((msg) => capturedAck = msg);
      handler.onQueueSyncReceived((_, fromDeviceAddress) {
        capturedDeviceAddress = fromDeviceAddress;
      });
      ...
      expect(result, ProtocolMessageHandler.queueSyncHandledMarker);
      expect(capturedAck, isNotNull);
      expect(capturedAck!.type, ProtocolMessageType.ack);
      expect(capturedDeviceAddress, 'device-sync');
    });
