// Phase 12.10: PairingService supplementary coverage
...
void main() {
  ...
  setUp(() {
    ...
    service = PairingService(
      getMyPersistentId: () async => 'my_persistent_id',
  ...
  group('PairingService - initiatePairingRequest', () {
    test('sets state to pairingRequested and sends protocol message', () {
      service.initiatePairingRequest(
        myEphemeralId: 'my_eph_id',
  ...
  group('PairingService - receivePairingRequest', () {
    test('sets state to requestReceived', () {
      service.receivePairingRequest(
        theirEphemeralId: 'peer_eph',
        displayName: 'Peer',
      );
  ...
  group('PairingService - _performVerification edge cases', () {
    test('verification fails when theirSessionId is null', () async {
      final nullSessionService = PairingService(
        getMyPersistentId: () async => 'my_id',
        getTheirSessionId: () => null,
        getTheirDisplayName: () => 'Peer',
      );
      nullSessionService.onSendPairingCode = (code) => sentCodes.add(code);

      nullSessionService.receivePairingRequest(
        theirEphemeralId: 'peer_eph',
        displayName: 'Peer',
      );
      nullSessionService.acceptIncomingRequest(
        myEphemeralId: 'my_eph',
        displayName: 'Me',
      );
      ...
      expect(
        nullSessionService.currentPairing!.state,
  ...
  group('PairingService - setCurrentPairing', () {
    ...
    test('can set to null', () {
      service.setCurrentPairing(
        PairingInfo(myCode: '1234', state: PairingState.displaying),
      );
