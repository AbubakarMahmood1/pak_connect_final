/// OutboundMessageSender integration tests covering
...
class _FakeSecurityService extends Fake implements ISecurityService {
 EncryptionMethod nextMethod = EncryptionMethod.noise('default-noise-session-key',
);
...
void main() {
 ...
 final sender = OutboundMessageSender(logger: logger,
 ...
 final result = await sender.sendCentralMessage(centralManager: CentralManager(),
 ...
 expect(logs.where((log) => log.message.contains('node-secret-abcdef1234567890'),
),
 isEmpty,
);
 expect(logs.where((log) => log.message.contains('SEND DEBUG: Message content')),
 isEmpty,
);
 });
 ...
 try {
 await sender.sendCentralMessage(centralManager: CentralManager(),
 connectedDevice: fakePeripheral,
 messageCharacteristic: fakeCharacteristic,
 message: 'test msg',
 mtuSize: 512,
 ...
 contactRepo.contactToReturn = _makeContact(publicKey: 'spy-contact-pk-1234567890ab',
 ...
 final spyLogs = logs.where((l) => l.message.contains('SPY MODE'),
);
 ...
 expect(result, isTrue);
 expect(logs.any((l) => l.message.contains('Failed to get security level')),
 isTrue,
);
 });
 ...
 expect(logs.any((l) => l.message.contains('Message timeout')),
 isTrue,
);
 });
 ...
 expect(result, isFalse);
 expect(logs.any((l) => l.message.contains('SEND STEP 6.1')),
 isTrue,
 ...
 expect(logs.any((l) => l.message.contains('Using binary envelope')),
 isTrue,
);
 });
 ...
 expect(logs.any((l) => l.message.contains('Chunk fragmentation failed'),
),
 isTrue,
 ...
 final result = await sender.sendPeripheralMessage(peripheralManager: PeripheralManager(),
 ...
 expect(logs.any((l) => l.message.contains('Outbound message prepared')),
 isTrue,
);
 });
 ...
 try {
 await sender.sendPeripheralMessage(peripheralManager: PeripheralManager(),
 connectedCentral: fakeCentral,
 messageCharacteristic: fakeCharacteristic,
 message: 'will fail',
 mtuSize: 512,
 ...
 securityService.nextMethod = EncryptionMethod.ecdh('sealed-contact-pk-abcdef1234',
);
 // Provide a contact with a valid 32-byte base64 noise key
 final noiseKey = base64.encode(List.filled(32, 0x42));
 contactRepo.contactToReturn = _makeContact(publicKey: 'sealed-contact-pk-abcdef1234',
 ...
 expect(result, isTrue);
 expect(logs.any((l) => l.message.contains('SEALED_V1 offline lane')),
 isTrue,
);
 });
 ...
 expect(result, isTrue);
 expect(logs.any((l) => l.message.contains('Peripheral message dispatched')),
 isTrue,
);
 });
 ...
 expect(logs.any((l) => l.message.contains('binary envelope')),
 isTrue,
);
 });
 ...
 securityService.nextMethod = EncryptionMethod.noise('noise-pk-1234567890abcdef',
);

 contactRepo.contactToReturn = _makeContact(publicKey: 'noise-contact-pk-abcdef1234',
 ...
 expect(result, isTrue);
 expect(logs.any((l) => l.message.contains('Encrypted with NOISE method'),
),
 isTrue,
);
 });
 ...
 test('sealed V1 applied when legacy transport falls back to offline lane', () async {
 securityService.nextMethod = EncryptionMethod.ecdh('sealed-central-pk-abcdef1234',
);
 final noiseKey = base64.encode(List.filled(32, 0x42));
 contactRepo.contactToReturn = _makeContact(publicKey: 'sealed-central-pk-abcdef1234',
 noisePublicKey: noiseKey,
);
 contactRepo.contactByAnyIdToReturn = contactRepo.contactToReturn;
