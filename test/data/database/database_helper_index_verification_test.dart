...
void main() {
 ...
 final indexes = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='contacts'",
);
 ...
 final db = await DatabaseHelper.database;
 final indexes = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='chats'",
);
 final names = indexes.map((i) => i['name'] as String).toSet();
 ...
 final db = await DatabaseHelper.database;
 final indexes = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='messages'",
);
 final names = indexes.map((i) => i['name'] as String).toSet();
 ...
 final db = await DatabaseHelper.database;
 final indexes = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='offline_message_queue'",
);
 final names = indexes.map((i) => i['name'] as String).toSet();
 ...
 final db = await DatabaseHelper.database;
 final indexes = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='seen_messages'",
);
 final names = indexes.map((i) => i['name'] as String).toSet();
 ...
 final members = await db.query('group_members',
 where: 'group_id = ?',
 whereArgs: ['grp-cascade-1'],
);
 ...
 final msgs = await db.query('group_messages',
 where: 'group_id = ?',
 whereArgs: ['grp-msg-cascade'],
);
 expect(msgs, isEmpty);
 ...
 final deliveries = await db.query('group_message_delivery',
 where: 'message_id = ?',
 whereArgs: ['gmsg-delivery-1'],
);
 ...
 final rows = await db.query('queue_sync_state',
 where: 'device_id = ?',
 ...
 final rows = await db.query('deleted_message_ids',
 where: 'message_id = ?',
 whereArgs: ['del-msg-1'],
);
 expect(rows, hasLength(1));
 ...
 final rows = await db.query('migration_metadata',
 where: '"key" = ?',
 ...
 final threadMsgs = await db.query('messages',
 where: 'thread_id = ?',
 whereArgs: ['thread-1'],
