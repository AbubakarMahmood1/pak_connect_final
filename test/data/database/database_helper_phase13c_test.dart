...
Future<sqflite_common.Database> _openRawDb(
  String name, {
...
void main() {
  ...
  group('testOnConfigure', () {
    test('enables foreign keys, WAL mode and cache size', () async {
      ...
      expect(
        logRecords.any((l) => l.message.contains('WAL mode set')),
        isTrue,
      );
      expect(
        logRecords.any(
          (l) => l.message.contains('Database configured'),
        ),
        isTrue,
      );
    });
  ...
  group('testOnUpgrade', () {
    test('v9 to v10 adds seen_messages table', () async {
      final db = await _openRawDb(
        ...
        onCreate: (db, version) async {
          await DatabaseSchemaBuilder.createSchema(
            db,
            version,
      ...
      final idx = await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='index' AND name LIKE 'idx_seen_%'",
      );
    ...
    test('full migration v1 to v10 runs all steps', () async {
      ...
      final groups = await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='contact_groups'",
      );
      expect(groups, isNotEmpty);

      // Verify v10: seen_messages
      final seen = await db.rawQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='seen_messages'",
      );
  ...
  group('close', () {
    test('closes the database', () async {
      ...
      expect(
        logRecords.any((l) => l.message.contains('Database closed')),
        isTrue,
      );
    });
  ...
  group('testIsDatabaseEncrypted', () {
    test('returns false when file does not exist', () async {
      final result = await DatabaseHelper.testIsDatabaseEncrypted(
    ...
    test('returns false for plaintext SQLite file', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('plaintext SQLite (not encrypted)'),
        ),
        isTrue,
      );
    ...
    test('returns true for non-SQLite (encrypted-looking) file', () async {
      ...
      final bytes = Uint8List.fromList(
        List.generate(64, (i) => (i * 37 + 13) % 256),
      );
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('appears to be encrypted'),
        ),
        isTrue,
      );
  ...
  group('testCopyDatabaseContents', () {
    Future<sqflite_common.Database> createSimpleDb(
      String name, {
    ...
    test('copies rows from source tables to destination', () async {
      ...
      expect(
        logRecords.any((l) => l.message.contains('Migration complete')),
        isTrue,
      );
    });
    ...
    test('skips FTS tables', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('Skipping FTS table'),
        ),
        isTrue,
      );
    });
    ...
    test('skips tables not present in destination', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('Skipping table legacy_data'),
        ),
        isTrue,
      );
    });
    ...
    test('handles empty source tables', () async {
      ...
      expect(
        logRecords.any((l) => l.message.contains('is empty')),
        isTrue,
      );
    });
  });
  ...
  group('testApplyDataMigrationBackfills', () {
    test('backfills current_ephemeral_id from ephemeral_id', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('v8 backfill complete'),
        ),
        isTrue,
      );
    });
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains(
            'current_ephemeral_id column not found',
          ),
        ),
        isTrue,
      );
  ...
  group('testRebuildFtsIndexes', () {
    test('rebuilds FTS when archived messages exist', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('FTS rebuild complete'),
        ),
        isTrue,
      );
    });
    ...
    test('skips FTS rebuild when no archived messages', () async {
      ...
      expect(
        logRecords.any(
          (l) => l.message.contains('No archived messages to index'),
        ),
        isTrue,
      );
    });
