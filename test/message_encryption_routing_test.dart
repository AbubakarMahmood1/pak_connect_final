...
void main() {
  group('Message Encryption and Routing Tests', () {
      ...
      test('should create message with correct intended recipient', () async {
        ...
        final protocolMessage = ProtocolMessage(
          ...
          payload: {
            ...
            'intendedRecipient': arshadNodeId, // Key test: recipient should be Arshad
          },
          timestamp: DateTime.now(),
        );
        
        // Verify the intended recipient is correctly set
        expect(protocolMessage.payload['intendedRecipient'], equals(arshadNodeId));
      ...
      test('should block message when NOT intended for current user', () async {
        ...
        final messageJson = jsonEncode({
          ...
          'payload': {
            ...
            'encrypted': false,
            'encryptionMethod': 'none',
            'intendedRecipient': arshadNodeId, // Intended for Arshad, not Abubakar
          },
          'timestamp': DateTime.now().millisecondsSinceEpoch,
          'useEphemeralSigning': false,
        });
    ...
    group('2. Message Encryption Tests', () {
      ...
      test('should process encrypted message when intended for current user', () async {
        // Set Arshad as current node (recipient)
        messageHandler.setCurrentNodeId(arshadNodeId);
    ...
    group('3. Relay Messaging Tests (Ali → Abubakar via Arshad)', () {
      test('should create relay message with correct final recipient', () async {
      ...
      test('should process relay message when current node is final recipient', () async {
        ...
        expect(result, isNull); // Returns null for relay messages but processes them
      });
    });

    group('4. Chat Screen Context Isolation Tests', () {
      test('should maintain separate recipient contexts for different chats', () {
        ...
        expect(aliToAbubakarContext['contactPublicKey'], equals(abubakarNodeId));
        expect(aliToArshadContext['chatId'], isNot(equals(aliToAbubakarContext['chatId'])));
      });
      ...
      test('should use correct recipient key based on chat context', () {
        ...
        expect(messageToArshad.payload['intendedRecipient'], equals(arshadNodeId));
        expect(messageToAbubakar.payload['intendedRecipient'], equals(abubakarNodeId));
      });
    });
    ...
    group('7. Message Handler Node ID Validation', () {
      test('should handle node ID bounds correctly', () {
        ...
        const longNodeId = 'very_long_node_id_that_exceeds_normal_bounds_12345678901234567890123456789012345678901234567890';
        
        expect(() => messageHandler.setCurrentNodeId(shortNodeId), returnsNormally);
        expect(() => messageHandler.setCurrentNodeId(normalNodeId), returnsNormally);
        expect(() => messageHandler.setCurrentNodeId(longNodeId), returnsNormally);
      });
      
      test('should safely truncate node IDs in logging', () async {
        // Set a very long node ID
        const longNodeId = 'very_long_node_id_that_could_cause_substring_errors_12345678901234567890123456789012345678901234567890';
