/// MeshRelayEngine additional coverage
...
void main() {
 ...
 MeshRelayEngine(repositoryProvider: repo,
 ...
 await engine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'sender',
);
 }
 ...
 // Deliver one to self
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me', msgId: 'good'),
 fromNodeId: 'sender',
);

 // Drop one (handshake type)
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other', msgId: 'bad_type'),
 fromNodeId: 'sender',
 ...
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other', msgId: 'dup_id'),
 fromNodeId: 'sender',
);
 ...
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me', msgId: 'm1'),
 fromNodeId: 's',
);
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'x', msgId: 'm2'),
 fromNodeId: 's',
 ...
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me'),
 fromNodeId: 'sender',
);
 ...
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other', msgId: 'dup'),
 fromNodeId: 'sender',
);
 ...
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other'),
 fromNodeId: 'sender',
 ...
 final r1 = await engine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'a',
);
 expect(r1.type, RelayProcessingType.deliveredToSelf);

 // Second time: duplicate
 final r2 = await engine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'a',
);
 ...
 final r1 = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me', msgId: 'id_1'),
 fromNodeId: 'a',
);
 final r2 = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me', msgId: 'id_2'),
 fromNodeId: 'a',
);
 expect(r1.type, RelayProcessingType.deliveredToSelf);
 ...
 final r = await engine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'relay',
);
 ...
 final msg = _relay(recipient: SpecialRecipients.broadcast,
 msgId: 'bcast_1',
 ttl: 5,
 hopCount: 1,
);
 ...
 final msg = _relay(recipient: 'far_node',
 msgId: 'flood_msg',
 ttl: 5,
 hopCount: 1,
);
 ...
 final r = await engine.createOutgoingRelay(originalMessageId: 'msg_$type',
 ...
 final r = await engine.createOutgoingRelay(originalMessageId: 'msg_text',
 ...
 final r = await engine.createOutgoingRelay(originalMessageId: 'msg_urg',
 ...
 final r = await engine.createOutgoingRelay(originalMessageId: 'msg_enc',
 ...
 final engine = makeEngine(repo: _AliasRepoProvider(_contact(publicKey: 'contact-root',
 ...
 final r = await engine.shouldAttemptDecryption(finalRecipientPublicKey: 'me',
 ...
 final r = await engine.shouldAttemptDecryption(finalRecipientPublicKey: 'stranger',
 ...
 test('efficiency = (relayed+delivered)/(relayed+delivered+dropped)',
 () async {
 final engine = makeEngine();
 await engine.initialize(currentNodeId: 'me');

 // 1 delivered
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'me', msgId: 'd1'),
 fromNodeId: 's',
);

 // 2 dropped (handshake type)
 for (var i = 0; i < 2; i++) {
 await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other', msgId: 'h$i'),
 fromNodeId: 's',
 messageType: ProtocolMessageType.noiseHandshake1,
);
 }
 ...
 test('persistent ID resolver delivers persistent-addressed relay to self',
 () async {
 MeshRelayEngine.configureDependencyResolvers(persistentIdResolver: () => 'persistent-me',
);

 String? delivered;
 final engine = MeshRelayEngine(messageQueue: queue,
 spamPrevention: spam,
 seenMessageStore: seenStore,
);
 await engine.initialize(currentNodeId: 'ephemeral-me',
 onDeliverToSelf: (id, content, sender) => delivered = content,
);

 final result = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'persistent-me'),
 fromNodeId: 'sender',
);
 ...
 MeshRelayEngine.configureDependencyResolvers(repositoryProviderResolver: () => null,
);
 final engine = MeshRelayEngine(messageQueue: queue,
 spamPrevention: spam,
 seenMessageStore: seenStore,
 ...
 MeshRelayEngine.configureDependencyResolvers(repositoryProviderResolver: () => throw Exception('boom'),
);
 final engine = MeshRelayEngine(messageQueue: queue,
 spamPrevention: spam,
 seenMessageStore: seenStore,
 ...
 MeshRelayEngine.configureDependencyResolvers(seenMessageStoreResolver: () => null,
);
 ...
 final r = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'node_b'),
 fromNodeId: 'sender',
);
 expect(r.type, RelayProcessingType.deliveredToSelf);
 });
 ...
 final r = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'node_a', msgId: 'old_id'),
 fromNodeId: 'sender',
 availableNextHops: [],
);
 ...
 final engine = MeshRelayEngine(seenMessageStore: badStore,
 messageQueue: queue,
 spamPrevention: spam,
);
 await engine.initialize(currentNodeId: 'me');

 final r = await engine.processIncomingRelay(relayMessage: _relay(recipient: 'other'),
 fromNodeId: 'sender',
);
