// OfflineMessageQueue, QueueStore, QueueSync coverage
...
void main() {
 ...
 test('initializePersistence falls back to in-memory on persistence failure',
 () async {
 ...
 final queue = OfflineMessageQueue(queueRepository: _FakeQueueRepository(),
 ...
 await queue.initialize(onStatsUpdated: (stats) => lastStats = stats,
);
 await queue.clearQueue();
 ...
 await queue.initialize(repositoryProvider: _FakeRepositoryProvider(),
);
 ...
 OfflineMessageQueue.configureDefaultRepositoryProvider(_FakeRepositoryProvider(),
);
 final queue = OfflineMessageQueue(retryScheduler: _FakeRetryScheduler(),
);
 await queue.initialize();
 ...
 final msg = QueueSyncMessage(nodeId: 'node_1',
 queueHash: 'hash_abc',
 messageIds: ['id1', 'id2'],
 syncTimestamp: now,
