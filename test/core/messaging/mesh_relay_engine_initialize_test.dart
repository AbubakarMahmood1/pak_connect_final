...
void main() {
 ...
 engine = MeshRelayEngine(seenMessageStore: seenStore,
 ...
 expect(() => engine.initialize(currentNodeId: 'persistent_mykey'),
 throwsA(isA<ArgumentError>()),
);
 });

 test('rejects Ed25519 containing node ID', () async {
 expect(() => engine.initialize(currentNodeId: 'myEd25519PublicKey'),
 throwsA(isA<ArgumentError>()),
);
 });

 test('flood mode ignores routing/topology', () async {
 final floodEngine = MeshRelayEngine(seenMessageStore: seenStore,
 messageQueue: messageQueue,
 spamPrevention: spamPrevention,
 forceFloodMode: true,
);
 ...
 for (var i = 0; i < 200; i++) {
 await spamPrevention.recordRelayOperation(fromNodeId: 'spammer',
 toNodeId: 'my_node_id',
 messageHash: 'hash_$i',
 messageSize: 100,
);
 ...
 final result = await floodEngine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'sender_1',
 availableNextHops: [],
);
 expect(result.type, RelayProcessingType.dropped);
 ...
 final badEngine = MeshRelayEngine(seenMessageStore: badStore,
 messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
 ...
 final result = await badEngine.processIncomingRelay(relayMessage: msg,
 fromNodeId: 'sender',
);
 ...
 final relay = await engine.createOutgoingRelay(originalMessageId: 'msg_urgent',
 ...
 final relay = await engine.createOutgoingRelay(originalMessageId: 'msg_enc',
 ...
 final should = await engine.shouldAttemptDecryption(finalRecipientPublicKey: 'my_node',
 ...
 await engine.initialize(currentNodeId: 'my_node');
 final should = await engine.shouldAttemptDecryption(finalRecipientPublicKey: 'someone_else',
 ...
 MeshRelayEngine.configureDependencyResolvers(seenMessageStoreResolver: () => _FakeSeenMessageStore(),
);
 // Engine created without explicit store should use resolver
 final resolvedEngine = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
 ...
 final e = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
 expect(e, isNotNull);
 MeshRelayEngine.clearDependencyResolvers();
 ...
 MeshRelayEngine.configureDependencyResolvers(seenMessageStoreResolver: () => throw Exception('boom'),
);
 final e = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
 ...
 await engine.processIncomingRelay(relayMessage: msg1,
 fromNodeId: 'sender',
);

 // Drop one (handshake type)
 final msg2 = _buildRelay(recipient: 'other', msgId: 'msg_hs');
 await engine.processIncomingRelay(relayMessage: msg2,
 fromNodeId: 'sender',
 messageType: ProtocolMessageType.noiseHandshake1,
);
...
class _FakeOfflineMessageQueue implements OfflineMessageQueueContract {
 ...
 @override
 Future<bool> changePriority(String messageId,
