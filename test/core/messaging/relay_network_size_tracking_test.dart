// Network-Size Adaptive Relay Testing
...
void main() {
 ...
 group('Relay Probability Calculation', () {
 ...
 group('Probabilistic Relay Behavior', () {
 late MeshRelayEngine relayEngine;
 late NetworkTopologyAnalyzer topologyAnalyzer;
 late InMemoryOfflineMessageQueue messageQueue;
 late SpamPreventionManager spamPrevention;
 ...
 await relayEngine.initialize(currentNodeId: 'test_node_relay',
 topologyAnalyzer: topologyAnalyzer,
);
 ...
 test('Large network (50 nodes) skips some messages probabilistically',
 () async {
 ...
 await relayEngine.initialize(currentNodeId: 'test_stats_node',
 topologyAnalyzer: topologyAnalyzer,
);
 });
 ...
 expect(stats.networkSize, equals(30));
 expect(stats.currentRelayProbability, equals(1.0));
 expect(stats.totalProbabilisticSkip,
 equals(0),
 ...
 await relayEngine.processIncomingRelay(relayMessage: relayMessage,
 fromNodeId: 'relay',
 ...
 final relayEngine = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);

 await relayEngine.initialize(currentNodeId: 'integration_node',
 topologyAnalyzer: topologyAnalyzer,
);
 ...
 expect(result.isSuccess || (result.reason?.contains('Probabilistic') ?? false),
 isTrue,
);
 ...
 test('Messages for current node always delivered (bypass probabilistic skip)',
 () async {
 final messageQueue = InMemoryOfflineMessageQueue();
 await messageQueue.initialize();
 ...
 await relayEngine.initialize(currentNodeId: currentNodeId,
 topologyAnalyzer: topologyAnalyzer,
);
