/// Phase 12.8: Tests for RelaySendPipeline, RelayDecisionEngine, QueuePolicyManager
...
void main() {
  ...
  group('RelaySendPipeline', () {
    ...
    test('relayToNextHop succeeds and calls onRelayMessage', () async {
      ...
      final ok = await pipeline.relayToNextHop(
        relayMessage: msg,
        nextHopNodeId: 'hop_1',
        onRelayMessage: (m, h) {
          captured = m;
    ...
    test('broadcastToNeighbors filters loop nodes', () async {
      ...
      final count = await pipeline.broadcastToNeighbors(
        relayMessage: msg,
  ...
  group('RelayDecisionEngine', () {
    ...
    test('isMessageForCurrentNode returns true for broadcast', () async {
      expect(
        await engine.isMessageForCurrentNode(SpecialRecipients.broadcast),
        isTrue,
      );
    });
    ...
    test('shouldProbabilisticallySkip never skips when isForUs', () {
      expect(
        engine.shouldProbabilisticallySkip(
          isForUs: true,
    ...
    test('chooseNextHop returns null for empty hops', () async {
      ...
      final hop = await engine.chooseNextHop(
        relayMessage: msg,
        availableHops: [],
      );
      expect(hop, isNull);
    ...
    test('chooseNextHopId wraps string result', () async {
      final msg = _buildRelay(recipient: 'dest');
      final hop = await engine.chooseNextHopId(
        relayMessage: msg,
  ...
  group('QueuePolicyManager', () {
    ...
    test('isContactFavorite delegates to repository', () async {
      final manager = QueuePolicyManager(
        repositoryProvider: _FakeRepoProvider(favoriteKeys: {'fav_pk'}),
      );
    ...
    test('applyFavoritesPriorityBoost does not boost non-favorites', () async {
      final manager = QueuePolicyManager(
        repositoryProvider: _FakeRepoProvider(favoriteKeys: {}),
      );
      final result = await manager.applyFavoritesPriorityBoost(
        recipientPublicKey: 'regular',
        currentPriority: MessagePriority.normal,
      );
      expect(result.wasBoosted, isFalse);
    ...
    test('validateQueueLimit allows when under limit', () async {
      final manager = QueuePolicyManager();
      final result = await manager.validateQueueLimit(
    ...
    test('isContactFavorite handles repository error gracefully', () async {
      final manager = QueuePolicyManager(
        repositoryProvider: _ThrowingRepoProvider(),
      );
      expect(await manager.isContactFavorite('pk'), isFalse);
    });
  });
...
QueuedMessage _buildQueuedMessage(
  String recipientKey,
  QueuedMessageStatus status,
) {
  return QueuedMessage(
    id: 'qm_${DateTime.now().microsecondsSinceEpoch}',
    chatId: 'chat_1',
    content: 'test',
