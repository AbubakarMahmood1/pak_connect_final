// Tests for BLE Integration and Message Type Filtering
...
void main() {
 ...
 final message = ProtocolMessage.meshRelay(originalMessageId: 'test-msg-123',
 ...
 test('Should handle meshRelay without message type (backward compatibility)',
 () {
 final message = ProtocolMessage.meshRelay(originalMessageId: 'test-msg-123',
 originalSender: 'sender-abc',
 finalRecipient: 'recipient-xyz',
 ...
 final message = ProtocolMessage.meshRelay(originalMessageId: 'test-msg',
 ...
 expect(decoded.meshRelayOriginalMessageType,
 equals(originalType),
 ...
 expect(nextHopMessage.originalMessageType,
 equals(ProtocolMessageType.textMessage),
);
 ...
 expect(decoded.originalMessageType,
 ...
 group('Relay Engine Message Type Filtering', () {
 ...
 relayEngine = MeshRelayEngine(messageQueue: messageQueue,
 ...
 test('Should prevent creating relay for non-eligible message type',
 () async {
 final relayMessage = await relayEngine.createOutgoingRelay(originalMessageId: 'msg-001',
 ...
 expect(relayMessage!.originalMessageType,
 equals(ProtocolMessageType.textMessage),
);
 });
 ...
 test('Should preserve message type through multi-hop relay chain',
 () async {
 ...
 final engineA = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
 ...
 for (int i = 0; i < 3; i++) {
 expect(RelayPolicy.isRelayEligibleMessageType(type),
 equals(isEligible),
 ...
 final engine = MeshRelayEngine(messageQueue: messageQueue,
 spamPrevention: spamPrevention,
);
