...
void main() {
  ...
  group('PRNG Security Fixes', () {
    ...
    test('signing same message twice produces different signatures (non-deterministic k-nonce)', () {
      ...
      expect(signature1, isNot(equals(signature2)),
        reason: 'ECDSA signatures must use random k-nonce, resulting in different signatures for same message');
    });
    ...
    test('no timestamp-seeded PRNG patterns in cryptographic source files', () {
      ...
      for (final filePath in criticalFiles) {
        ...
        final hasMillisecondsSinceEpoch = content.contains('millisecondsSinceEpoch');
        final hasMicrosecondsSinceEpoch = content.contains('microsecondsSinceEpoch');
        ...
        if (hasMillisecondsSinceEpoch || hasMicrosecondsSinceEpoch) {
          ...
          final hasCryptoContext = content.contains('FortunaRandom') || 
          ...
          if (hasCryptoContext) {
            ...
            for (var i = 0; i < lines.length; i++) {
              final line = lines[i];
              if ((line.contains('millisecondsSinceEpoch') || line.contains('microsecondsSinceEpoch'))) {
                // Check if within 10 lines of crypto code
                for (var j = (i - 10).clamp(0, lines.length); j < (i + 10).clamp(0, lines.length); j++) {
            ...
            expect(foundDangerousPattern, isFalse,
              reason: 'File $filePath contains timestamp-based PRNG seeding in cryptographic context');
          }
        }
      }
    });
  ...
  group('Archive PLAINTEXT Migration Fix', () {
    test('decryptField handles enc::archive::v1::PLAINTEXT: prefix correctly', () {
      ...
      expect(result, equals(testValue),
        reason: 'Should extract plaintext from enc::archive::v1::PLAINTEXT: format');
    });
    ...
    test('decryptField handles legacy AES encrypted values', () {
      ...
      const fakeLegacyEncrypted = 'enc::archive::v1::YWJjZGVmZ2g='; // base64 "abcdefgh"
    ...
    test('encryptField uses plaintext (no encryption)', () {
      ...
      expect(result, equals(testValue),
        reason: 'Archive encryption now stores plaintext (relies on DB encryption)');
    });
    ...
    test('handles multiple PLAINTEXT: patterns correctly', () {
      ...
      expect(result, equals(testValue),
        reason: 'Should only strip the first PLAINTEXT: marker after legacy prefix');
    });
  });
}
