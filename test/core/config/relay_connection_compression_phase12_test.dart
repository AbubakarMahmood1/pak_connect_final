// Phase 12.16b: RelayConfigManager + ConnectionLimitConfig + CompressionConfig
...
void main() {
  ...
  group('ChatUtils', () {
    test('resolveChatKey priority: persistent > session > ephemeral', () {
      ...
      expect(
        ChatUtils.resolveChatKey(currentEphemeralId: 'eid'),
        'eid',
      );
      expect(ChatUtils.resolveChatKey(), isNull);
      // Empty strings treated as absent
      expect(
        ChatUtils.resolveChatKey(persistentPublicKey: '', currentSessionId: ''),
        isNull,
      );
    });
    ...
    test('extractContactKey handles key1==myPublicKey branch', () {
      ...
      final result = ChatUtils.extractContactKey(
        'persistent_chat_mykey_contactkey',
        'mykey',
      );
      expect(result, 'contactkey');

      // When key2 matches myPublicKey, return key1
      final result2 = ChatUtils.extractContactKey(
        'persistent_chat_contactkey_mykey',
        'mykey',
      );
