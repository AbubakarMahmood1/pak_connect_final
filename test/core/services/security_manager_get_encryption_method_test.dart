...
Contact _makeContact(String pk) => Contact(publicKey: pk,
 displayName: 'Test',
...
class _FakeContactRepository extends Fake implements IContactRepository {
 ...
 @override
 Future<void> updateContactSecurityLevel(String publicKey,
 SecurityLevel level,
) async {
...
void main() {
 ...
 expect(() => sm.getEncryptionMethod('pk_low', repo),
 ...
 repo.setContact(Contact(publicKey: 'pk_high',
 displayName: 'Test',
 ...
 expect(() => sm.getEncryptionMethod('pk_high', repo),
 throwsA(isA<Exception>()),
);
 });
 ...
 expect(() => sm.getEncryptionMethod('pk_med', repo),
 throwsA(isA<Exception>()),
);
 });

 test('HIGH level with valid ECDH key and verified trust returns ecdh',
 () async {
 repo.setSecurityLevel(SecurityLevel.high);
 repo.setCachedSecret('some_shared_secret');
 repo.setContact(Contact(publicKey: 'pk_ecdh',
 displayName: 'Test',
 trustStatus: TrustStatus.verified,
 securityLevel: SecurityLevel.high,
 firstSeen: DateTime.now(),
 ...
 expect(() => sm.getEncryptionMethod('pk_pair', repo),
 throwsA(isA<Exception>()),
);
 });
 });
 ...
 expect(() => sm.encryptMessageByType('test message',
 'pk_global',
 repo,
 ...
 expect(() => sm.encryptMessageByType('test',
 'pk_noise',
 repo,
 ...
 await expectLater(() => sm.decryptMessageByType('PLAINTEXT:hello world',
 'pk_test',
 repo,
 EncryptionType.global,
),
 ...
 repo.setCachedSecret(null);
 expect(() => sm.decryptMessageByType('encrypted_ecdh',
 'pk_test',
 repo,
 EncryptionType.ecdh,
),
 ...
 expect(() => sm.decryptMessageByType('encrypted_pairing',
 'pk_test',
 repo,
 ...
 await expectLater(() => sm.decryptMessage('PLAINTEXT:secret message', 'pk_dec', repo),
 throwsA(isA<Exception>().having((error) => error.toString(),
 'message',
 ...
 // This is already tested in but we verify the path
 ...
 SecurityManager.configureContactRepositoryResolver(() => _FakeContactRepository(),
);
 ...
 expect(() => sm.encryptBinaryPayload(data, 'pk_bin', repo),
 throwsA(isA<Exception>()),
);
 });
 ...
 repo.setContact(Contact(publicKey: 'pk_bin_ecdh',
 displayName: 'Test',
 trustStatus: TrustStatus.verified,
 securityLevel: SecurityLevel.high,
 firstSeen: DateTime.now(),
 ...
 final data = Uint8List.fromList([1, 2, 3, 4, 5]);
 final encrypted = await sm.encryptBinaryPayload(data,
 'pk_bin_ecdh',
 repo,
);
 ...
 repo.setContact(Contact(publicKey: 'pk_bin_dec',
 displayName: 'Test',
 trustStatus: TrustStatus.verified,
 securityLevel: SecurityLevel.high,
 firstSeen: DateTime.now(),
 ...
 final decrypted = await sm.decryptBinaryPayload(encrypted,
 'pk_bin_dec',
 repo,
);
 ...
 expect(() => sm.decryptBinaryPayload(data, 'pk_test', repo),
 throwsA(isA<Exception>()),
);
 });
 });
