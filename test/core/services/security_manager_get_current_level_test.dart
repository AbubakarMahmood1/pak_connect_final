/// Additional coverage for SecurityManager focusing on
...
void main() {
 ...
 repo.byAnyId['pk-noe'] = _contact(key: 'pk-noe',
 trustStatus: TrustStatus.verified,
 securityLevel: SecurityLevel.high,
);
 ...
 repo.byAnyId['pk-sync'] = _contact(key: 'pk-sync',
 ...
 repo.byAnyId['pk-low'] = _contact(key: 'pk-low',
 trustStatus: TrustStatus.newContact,
 ...
 repo.byAnyId['pk-med'] = _contact(key: 'pk-med',
 trustStatus: TrustStatus.verified,
 ...
 repo.byAnyId['pk-bad'] = _contact(key: 'pk-bad',
 trustStatus: TrustStatus.verified,
 securityLevel: SecurityLevel.medium,
 ...
 final (pattern, _) = await sm.selectNoisePatternForUser(UserId('uid-2'),
 repo,
);
 expect(pattern, NoisePattern.xx);
 });
 ...
 expect(() => sm.encryptMessageByType('msg', 'pk', repo, EncryptionType.ecdh),
 throwsA(isA<EncryptionException>()),
);
 });
 });
 ...
 expect(() => sm.decryptMessageByType('data',
 'no-pairing-key',
 repo,
 ...
 expect(() => sm.decryptSealedMessage(encryptedMessage: 'ct',
 cryptoHeader: header,
 messageId: 'm1',
 senderId: 's1',
 recipientId: 'r1',
 ...
 repo.byAnyId['pk-bin'] = _contact(key: 'pk-bin',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.encryptBinaryPayload(Uint8List.fromList([1, 2, 3]),
 'pk-bin',
 repo,
),
 throwsA(isA<EncryptionException>()),
 ...
 expect(() => sm.registerIdentityMapping(persistentPublicKey: 'ppk-1',
 ephemeralID: 'eph-1',
),
 returnsNormally,
);
 ...
 expect(() => sm.registerIdentityMappingForUser(persistentUserId: UserId('u1'),
 ephemeralID: 'e1',
),
 returnsNormally,
);
 ...
 repo.byAnyId['pk-glo'] = _contact(key: 'pk-glo',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.getEncryptionMethod('pk-glo', repo),
 throwsA(isA<EncryptionException>()),
);
 });

 test('getEncryptionMethodForUser propagates missing active lane', () async {
 repo.byAnyId['uid-3'] = _contact(key: 'uid-3',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.getEncryptionMethodForUser(UserId('uid-3'), repo),
 throwsA(isA<EncryptionException>()),
);
 });
 });
 ...
 () async {
 repo.byAnyId['pk-gm'] = _contact(key: 'pk-gm',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.encryptMessage('hello', 'pk-gm', repo),
 throwsA(isA<EncryptionException>()),
);
 },
);
 ...
 repo.byAnyId['uid-4'] = _contact(key: 'uid-4',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.encryptMessageForUser('msg', UserId('uid-4'), repo),
 throwsA(isA<EncryptionException>()),
);
 });
 });
 ...
 repo.byAnyId['pk-fail'] = _contact(key: 'pk-fail',
 trustStatus: TrustStatus.newContact,
 securityLevel: SecurityLevel.low,
);
 expect(() => sm.decryptMessage('garbage', 'pk-fail', repo),
 throwsA(isA<Exception>()),
);
 });

 test('decryptMessageForUser delegates', () async {
 expect(() => sm.decryptMessageForUser('ct', UserId('uid-5'), repo),
 throwsA(isA<Exception>()),
);
 });
 });
