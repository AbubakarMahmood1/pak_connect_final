/// SecurityManager coverage targeting remaining uncovered lines:
///
...
void main() {
 ...
 expect(() => sm.initialize(secureStorage: failingStorage),
 ...
 repo.byAnyId[pk] = _contact(key: pk,
 ...
 test('MEDIUM: no pairing key, no noise session → missing active lane',
 () async {
 ...
 expect(() => sm.getEncryptionMethod(pk, repo),
 throwsA(isA<EncryptionException>()),
);
 },
);

 test('HIGH with ECDH → verified HIGH, ECDH verify succeeds → ecdh method',
 () async {
 ...
 try {
 await sm.encryptMessageByType('hello',
 'pk-noise-err-13c',
 repo,
 ...
 final encrypted = await SimpleCrypto.encryptForContact('test-message',
 pk,
 repo,
);
 if (encrypted == null) {
 ...
 test('valid sealed header but decryption fails → rethrow + finally cleanup (lines 542-570)',
 () async {
 ...
 try {
 await sm.decryptSealedMessage(encryptedMessage: base64.encode(List.filled(64, 0xCC)),
 cryptoHeader: header,
 ...
 test('contact with persistentPublicKey but no ephemeralId → uses persistent key (line 604)',
 () async {
 ...
 test('contact lookup throws → catch at line 609-610, then late-bind attempted',
 () async {
 repo.throwOnGetContact = true;
 try {
 await sm.encryptMessageByType('test',
 ...
 test('contact with ephemeralId set → uses ephemeralId as sessionId',
 () async {
 ...
 test('noise: _noiseService null → EncryptionException (lines 682-688)',
 () async {
 sm.shutdown();
 ...
 test('noise: no established session → EncryptionException (lines 689-696)',
 () async {
