// Phase 12.15: SecurityManager supplementary coverage
...
void main() {
  ...
  group('SecurityManager — identity mapping null guards', () {
    ...
    test('registerIdentityMappingForUser delegates without error', () {
      expect(
        () => sm.registerIdentityMappingForUser(
          persistentUserId: const UserId('user1'),
          ephemeralID: 'eph1',
        ),
        returnsNormally,
      );
    ...
    test('unregisterIdentityMappingForUser delegates without error', () {
      expect(
        () => sm.unregisterIdentityMappingForUser(const UserId('user1')),
        returnsNormally,
      );
    });
  });
  ...
  group('SecurityManager — decryptMessage fallback chain', () {
    ...
    test('decryptMessage throws when all methods fail', () async {
      ...
      expect(
        () => sm.decryptMessage('invalid-cipher', pk, repo),
        throwsA(isA<Exception>()),
      );
    });
  });
