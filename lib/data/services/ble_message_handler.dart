...
class BLEMessageHandler {
  ...
  onSendAckMessage; // Callback for sending ACK protocol messages
  Function(ProtocolMessage relayMessage, String nextHopId)?
  onSendRelayMessage; // Callback for forwarding relay messages
  ...
  Future<void> initializeRelaySystem({
    ...
    try {
      ...
      print(
        '🔧 INIT DEBUG: Setting current node ID to: ${_safeTruncate(currentNodeId, 16)}...',
      );
    ...
    try {
      ...
      final protocolMessage = ProtocolMessage.textMessage(
        ...
        useEphemeralAddressing:
            useEphemeralAddressing, // STEP 7: Include addressing flag
      );
      ...
      print(
        '🔧 SEND DIAGNOSTIC: Current node length: ${_currentNodeId?.length ?? 0}',
      );
      ...
      print(
        '🔧 SEND DEBUG: Current node ID: ${_safeTruncate(_currentNodeId, 16, fallback: "NOT SET")}...',
      );
      ...
      print(
        '📨 SEND STEP 2a: First 20 bytes: ${jsonBytes.sublist(0, jsonBytes.length > 20 ? 20 : jsonBytes.length)}',
      );
      ...
      for (int i = 0; i < chunks.length; i++) {
        ...
        print(
          '📨 SEND STEP 5.${i + 1}: Converting chunk ${i + 1}/${chunks.length} to bytes',
        );
        print(
          '📨 SEND STEP 5.${i + 1}a: Chunk format: ${chunk.messageId}|${chunk.chunkIndex}|${chunk.totalChunks}|${chunk.isBinary ? "1" : "0"}|[${chunk.content.length} chars]',
        );

        final chunkData = chunk.toBytes();
        print(
          '📨 SEND STEP 5.${i + 1}b: Chunk ${i + 1} → ${chunkData.length} bytes',
        );
        print(
          '📨 SEND STEP 5.${i + 1}c: First 50 bytes: ${chunkData.sublist(0, chunkData.length > 50 ? 50 : chunkData.length)}',
        );

        _logger.info(
          'Sending central chunk ${i + 1}/${chunks.length} for message: $msgId',
        );

        print(
          '📨 SEND STEP 6.${i + 1}: Writing chunk ${i + 1} to BLE characteristic',
        );
        await centralManager.writeCharacteristic(
          connectedDevice,
          messageCharacteristic,
        ...
        print(
          '📨 SEND STEP 6.${i + 1}✅: Chunk ${i + 1} written to BLE successfully',
        );
  ...
  Future<bool> sendPeripheralMessage({
    ...
    try {
      ...
      for (int i = 0; i < chunks.length; i++) {
        ...
        _logger.info(
          'Sending peripheral chunk ${i + 1}/${chunks.length} for message: $msgId',
        );
    ...
    try {
      ...
      try {
        ...
        if (ProtocolMessage.isProtocolMessage(directMessage)) {
          ...
          return await _handleDirectProtocolMessage(
            directMessage,
            onMessageIdFound,
            senderPublicKey,
          );
        ...
        try {
          ...
          if (completeMessageBytes != null) {
            ...
            try {
              ...
              return await _processProtocolMessageDirect(
                protocolMessage,
                onMessageIdFound,
                senderPublicKey,
              );
  ...
  Future<String?> _handleDirectProtocolMessage(
    String jsonMessage,
    String? Function(String)? onMessageIdFound,
    String? senderPublicKey,
  ) async {
    try {
      ...
      switch (protocolMessage.type) {
        ...
        case ProtocolMessageType.textMessage:
          ...
          return await _processCompleteProtocolMessage(
            jsonMessage,
            onMessageIdFound,
            senderPublicKey,
          );
        ...
        case ProtocolMessageType.queueSync:
          ...
          final messageIds =
              (protocolMessage.payload['messageIds'] as List<dynamic>?)
                  ?.cast<String>();
        ...
        default:
          _logger.warning(
            'Unexpected direct protocol message type: ${protocolMessage.type}',
          );
          return null;
      }
    } catch (e) {
    ...
    try {
      switch (protocolMessage.type) {
        ...
        case ProtocolMessageType.textMessage:
          // Text messages should go through the complete message processing pipeline
          // where routing validation and decryption happens
          return await _processCompleteProtocolMessageDirect(
            protocolMessage,
            onMessageIdFound,
            senderPublicKey,
          );
      ...
      switch (protocolMessage.type) {
        case ProtocolMessageType.textMessage:
          ...
          final encryptionMethod =
              protocolMessage.payload['encryptionMethod'] as String?;
          ...
          print(
            '🔧 ROUTING DIAGNOSTIC: Message ID length: ${messageId.length}',
          );
          print(
            '🔧 ROUTING DIAGNOSTIC: Sender key length: ${senderPublicKey?.length ?? 0}',
          );
          print(
            '🔧 ROUTING DIAGNOSTIC: Intended recipient length: ${intendedRecipient?.length ?? 0}',
          );
          print(
            '🔧 ROUTING DIAGNOSTIC: Current node length: ${_currentNodeId?.length ?? 0}',
          );

          print(
            '🔧 ROUTING DEBUG: Message ID: ${_safeTruncate(messageId, 16)}...',
          );
          print(
            '🔧 ROUTING DEBUG: Sender public key: ${_safeTruncate(senderPublicKey, 16, fallback: "NULL")}...',
          );
          print(
            '🔧 ROUTING DEBUG: Intended recipient: ${_safeTruncate(intendedRecipient, 16, fallback: "NOT SPECIFIED")}...',
          );
          print(
            '🔧 ROUTING DEBUG: Current node ID: ${_safeTruncate(_currentNodeId, 16, fallback: "NOT SET")}...',
          );
          print('🔧 ROUTING DEBUG: Encryption method: $encryptionMethod');
          print(
            '🔧 ROUTING DEBUG: Message encrypted flag: ${protocolMessage.isEncrypted}',
          );
          ...
          if (senderPublicKey != null &&
            ...
            print(
              '🔧 ROUTING DEBUG: 🚫 BLOCKING OWN MESSAGE - Sender matches current user',
            );
            return null;
          }
          ...
          if (intendedRecipient != null) {
            ...
            if (!isForMe) {
              print(
                '🔧 ROUTING DEBUG: 🚫 DISCARDING - Message not addressed to any of our identities',
              );
              print(
                '🔧 ROUTING DEBUG: - Intended recipient: ${_safeTruncate(intendedRecipient, 16)}...',
              );
              return null; // Not for us, discard
            }
            print(
              '🔧 ROUTING DEBUG: ✅ MESSAGE FOR US - Matched one of our identities',
            );
          } else {
            print(
              '🔧 ROUTING DEBUG: ✅ DIRECT MESSAGE - No routing info, accepting',
            );
          }
          ...
          if (protocolMessage.isEncrypted &&
            ...
            _logger.warning(
              '🔒 MESSAGE: Encrypted but no sender key available',
            );
          ...
          if (protocolMessage.signature != null) {
            ...
            if (protocolMessage.useEphemeralSigning) {
              ...
              if (protocolMessage.ephemeralSigningKey == null) {
                _logger.warning(
                  '⚠️ Ephemeral message missing signing key - accepting without verification',
                );
            ...
            if (protocolMessage.useEphemeralSigning) {
              _logger.info(
                '✅ Ephemeral signature verified - message authentic but anonymous',
              );
            } else {
              _logger.info(
                '✅ Real signature verified - message authentic and identified',
              );
            }
          }
        ...
        case ProtocolMessageType.contactRequest:
          ...
          if (requestPublicKey != null && requestDisplayName != null) {
            _logger.info(
              '📱 CONTACT REQUEST: Received from $requestDisplayName',
            );
  ...
  Future<String?> _handleQueueSync(
    ...
    try {
      ...
      _logger.info(
        '🔄 QUEUE SYNC: Received ${syncMessage.syncType.name} from ${_safeTruncate(senderPublicKey, 16)}...',
      );
  ...
  Future<String?> _handleMeshRelay(
    ProtocolMessage protocolMessage,
    String? senderPublicKey,
  ) async {
    try {
      ...
      final originalMessageType = protocolMessage
          .meshRelayOriginalMessageType; // PHASE 2: Extract message type
      ...
      _logger.info(
        '🔀 MESH RELAY: Processing message ${_safeTruncate(originalMessageId, 16)}... from ${_safeTruncate(senderPublicKey, 8)}...',
      );
      ...
      final result = await _relayEngine!.processIncomingRelay(
        ...
        messageType:
            originalMessageType, // PHASE 2: Pass to relay engine for policy check
      );
      ...
      switch (result.type) {
        ...
        case RelayProcessingType.relayed:
          _logger.info(
            '🔀 MESH RELAY: Message relayed to ${_safeTruncate(result.nextHopNodeId, 8)}...',
          );
  ...
  Future<void> _handleRelayToNextHop(
    ...
    try {
      _logger.info(
        '🔀 RELAY FORWARD: Preparing to send relay message to ${_safeTruncate(nextHopNodeId, 8)}...',
      );
      ...
      final protocolMessage = ProtocolMessage.meshRelay(
        ...
        originalMessageType:
            message.originalMessageType, // PHASE 2: Preserve message type
      );
      ...
      if (onSendRelayMessage != null) {
        ...
        _logger.info(
          '✅ Relay message forwarded to ${_safeTruncate(nextHopNodeId, 8)}',
        );
      } else {
        _logger.warning(
  ...
  void _handleRelayDeliveryToSelf(
    ...
    try {
      _logger.info(
        '🔀 RELAY DELIVERY: Message delivered to self from ${_safeTruncate(originalSender, 8)}...',
      );
