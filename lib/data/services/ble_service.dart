...
class BLEService {
  ...
  final BluetoothStateMonitor _bluetoothStateMonitor =
      BluetoothStateMonitor.instance;
  ...
  bool _peripheralHandshakeStarted =
      false; // Track if handshake initiated for this connection
  bool _meshNetworkingStarted =
      false; // ✅ FIX: Track if mesh networking has been started at least once
  ...
  bool get isConnected {
    ...
    if (result) {
      ...
      _logger.fine(
        '💬 Connected ($sessionType): ${_stateManager.otherUserName ?? "unknown"}',
      );
    }
  ...
  Future<void> initialize() async {
    try {
      ...
      if (peripheralManager.state == BluetoothLowEnergyState.poweredOn &&
          _stateManager.isPeripheralMode) {
      ...
      if (_bluetoothStateMonitor.isBluetoothReady) {
        ...
        try {
          await _connectionManager.startMeshNetworking(
            onStartAdvertising: () async {
              _logger.info(
                '📡 [MESH-INIT] Starting peripheral mode via AdvertisingManager...',
              );
              await startAsPeripheral();
              _logger.info(
                '✅ [MESH-INIT] Peripheral mode started successfully',
              );
            },
          );
          ...
          _logger.severe(
            '❌ [MESH-INIT] Failed to start mesh networking: $e',
            e,
            stack,
          );
        ...
        _logger.info(
          '   Will start automatically when Bluetooth becomes available',
        );
      }

      // Connect message handler callbacks to state manager
      _messageHandler.onContactRequestReceived =
          _stateManager.handleContactRequest;
      _messageHandler.onContactAcceptReceived =
          _stateManager.handleContactAccept;
      _messageHandler.onContactRejectReceived =
      ...
      try {
        ...
        _logger.warning(
          '⚠️ EphemeralKeyManager not ready yet - will initialize later: $e',
        );
      ...
      _logger.info(
        '🔄 Initializing GossipSyncManager for mesh message discovery...',
      );

      // ✅ FIX #4: Safety check for AppCore initialization
      if (!AppCore.instance.isInitialized) {
        _logger.severe(
          '❌ AppCore not initialized yet - cannot access messageQueue',
        );
        _logger.severe(
          '   BLEService.initialize() must be called AFTER AppCore.initialize() completes',
        );
        throw StateError(
      ...
      _gossipSyncManager = GossipSyncManager(
        myNodeId:
            myEphemeralId ??
            'temp_node_id', // Temporary ID until EphemeralKeyManager is ready
      ...
      _gossipSyncManager!.onSendSyncRequest = (syncRequest) async {
        ...
        _logger.info(
          '📡 Gossip sync: Broadcasting sync request with ${syncRequest.messageIds.length} known messages',
        );
      ...
      if (_bluetoothStateMonitor.isBluetoothReady &&
        ...
        _logger.info(
          '⏳ GossipSyncManager created but not started (waiting for Bluetooth/connection)',
        );
      ...
      _logger.info(
        '📤 Offline message queue integration ready for handshake flush',
      );
      ...
      _connectionManager.onConnectionChanged = (device) {
        ...
        _logger.info(
          'DEBUG: Emitting connection state: $isConnected (device: ${device?.uuid})',
        );
      ...
      _stateManager.onNameChanged = (name) {
        ...
        _logger.info(
          '  Current connection state: isConnected=${_currentConnectionInfo.isConnected}, isReady=${_currentConnectionInfo.isReady}',
        );
        _logger.info(
          '  Current mode: ${_stateManager.isPeripheralMode ? "PERIPHERAL" : "CENTRAL"}',
        );
        ...
        try {
          ...
          } else if (_connectionManager.hasBleConnection &&
              _connectionManager.messageCharacteristic != null) {
      ...
      _logger.severe(
        '❌ CRITICAL: BLEService initialization failed',
        e,
        stackTrace,
      );
  ...
  void _handleMutualConsentRequired(String publicKey, String displayName) {
    ...
    _logger.info(
      '📱 MUTUAL CONSENT: User needs to decide whether to send contact request to $displayName',
    );
  ...
  void _handleAsymmetricContact(String publicKey, String displayName) {
    ...
    _logger.info(
      'Asymmetric contact detected: $displayName has us but we don\'t have them',
    );
  ...
  void _updateConnectionInfo({
    ...
    final effectiveAdvertising =
        isAdvertising ?? _authoritativeAdvertisingState;

    _logger.fine('🔍 CONNECTION INFO UPDATE REQUEST:');
    _logger.fine(
      '  - Input: isConnected=$isConnected, isReady=$isReady, otherUserName="$otherUserName"',
    );
    _logger.fine(
      '  - Input: statusMessage="$statusMessage", isScanning=$isScanning, isAdvertising=$isAdvertising (effective: $effectiveAdvertising), isReconnecting=$isReconnecting',
    );
    _logger.fine(
      '  - Current: isConnected=${_currentConnectionInfo.isConnected}, isReady=${_currentConnectionInfo.isReady}, otherUserName="${_currentConnectionInfo.otherUserName}"',
    );
    _logger.fine(
      '  - Current: isScanning=${_currentConnectionInfo.isScanning}, isAdvertising=${_currentConnectionInfo.isAdvertising}',
    );
    ...
    _logger.fine(
      '  - New Info: isConnected=${newInfo.isConnected}, isReady=${newInfo.isReady}, otherUserName="${newInfo.otherUserName}"',
    );
    _logger.fine(
      '  - New Info: isScanning=${newInfo.isScanning}, isAdvertising=${newInfo.isAdvertising}',
    );
    ...
    if (_shouldEmitConnectionInfo(newInfo)) {
      ...
      _logger.fine(
        '  - Final State: ${_currentConnectionInfo.isConnected}/${_currentConnectionInfo.isReady} - "${_currentConnectionInfo.statusMessage}"',
      );
      _logger.fine(
        '  - Final State: isScanning=${_currentConnectionInfo.isScanning}, isAdvertising=${_currentConnectionInfo.isAdvertising}',
      );
    } else {
    ...
    DeviceDeduplicationManager
      ...
      try {
        ...
        if (!autoConnectEnabled) {
          _logger.info(
            '🔗 AUTO-CONNECT: Disabled in settings - skipping $contactName',
          );
          return;
        }

        _logger.info(
          '🔗 AUTO-CONNECT: Enabled for $contactName - checking prerequisites...',
        );
        ...
        _logger.info(
          '✅ AUTO-CONNECT: Slots available ($currentSlots/$maxSlots) for $contactName',
        );

        // Check if already connected
        final alreadyConnected = _connectionManager.clientConnections.any(
        ...
        _logger.info(
          '✅ AUTO-CONNECT: Not yet connected to $contactName - proceeding...',
        );
  ...
  void _setupEventListeners() {
    ...
    centralManager.stateChanged.listen((event) async {
      ...
      if (event.state == BluetoothLowEnergyState.poweredOff) {
        ...
        if (hasActiveSession) {
          _logger.fine(
            '🔌 Active session detected - clearing session state (Central listener)',
          );
          _stateManager.clearSessionState();
        } else {
          _logger.fine(
            '🔵 No active session - skipping session clear (Central listener)',
          );
        }
    ...
    peripheralManager.stateChanged.listen((event) async {
      ...
      if (event.state == BluetoothLowEnergyState.poweredOff) {
        ...
        if (hasActiveSession) {
          _logger.fine(
            '🔌 Active session detected - clearing session state (Peripheral listener)',
          );
          _stateManager.clearSessionState();
        } else {
          _logger.fine(
            '🔵 No active session - skipping session clear (Peripheral listener)',
          );
        }
      ...
      if (event.state == BluetoothLowEnergyState.poweredOn &&
          _stateManager.isPeripheralMode) {
        _logger.info(
          '🔄 Bluetooth restarted in peripheral mode - restarting advertising...',
        );
        ...
        try {
          ...
          await _connectionManager.startMeshNetworking(
            onStartAdvertising: () async {
              _logger.info(
                '📡 [AUTO-RESTART] Starting peripheral mode via AdvertisingManager...',
              );
              await startAsPeripheral();
              _logger.info(
                '✅ [AUTO-RESTART] Peripheral mode started successfully',
              );
            },
          );
          ...
          _updateConnectionInfo(
            isAdvertising: true,
            isConnected: false,
            statusMessage: 'Advertising - waiting for connection',
          );
        } catch (e) {
      ...
      if (event.state == BluetoothLowEnergyState.poweredOff) {
        ...
        if (hasActiveSession) {
          _logger.fine(
            '🔌 Active session detected - clearing session state (Peripheral listener #2)',
          );
          _stateManager.clearSessionState();
        } else {
          _logger.fine(
            '🔵 No active session - skipping session clear (Peripheral listener #2)',
          );
        }
    ...
    peripheralManager.mtuChanged.listen((event) {
      _logger.info(
        '✅ Peripheral MTU changed: ${event.mtu} for ${event.central.uuid}',
      );
    ...
    centralManager.discovered.listen((event) async {
      ...
      _logger.info(
        '   - Manufacturer data: ${event.advertisement.manufacturerSpecificData.length} entries',
      );

      if (event.advertisement.manufacturerSpecificData.isNotEmpty) {
        for (
          var i = 0;
          i < event.advertisement.manufacturerSpecificData.length;
          i++
        ) {
          final mfg = event.advertisement.manufacturerSpecificData[i];
          _logger.info(
            '     [$i] ID=0x${mfg.id.toRadixString(16)}, Data=${mfg.data.length} bytes',
          );
        }
      }
      _logger.info(
      ...
      if (mfgData.isNotEmpty) {
        _logger.info(
          '🔍 [DISCOVERY-DEBUG] Checking manufacturer data for hints...',
        );
        for (final data in mfgData) {
          if (data.id == 0x2E19 && data.data.length == 15) {
            _logger.info(
              '🔍 [DISCOVERY-DEBUG] Found PakConnect hint data (0x2E19, 15 bytes)',
            );
            ...
            if (match.isContact) {
              _logger.info(
                '✅✅✅ [DISCOVERY-DEBUG] CONTACT NEARBY: ${match.contactName} ✅✅✅',
              );
              _hintMatchController?.add(
                '✅ Contact nearby: ${match.contactName}',
              );
            } else if (match.isIntro) {
              _logger.info('👋 INTRO MATCH: ${match.introHint?.displayName}');
              _hintMatchController?.add(
                '👋 Found: ${match.introHint?.displayName} (from QR)',
              );
            }
          }
        }
    ...
    centralManager.connectionStateChanged.listen((event) {
      _logger.info(
        'Connection state: ${event.peripheral.uuid} → ${event.state}',
      );
    ...
    if (Platform.isAndroid) {
      peripheralManager.connectionStateChanged.listen((event) {
        _logger.info(
          'Peripheral connection state: ${event.central.uuid} → ${event.state}',
        );

        if (event.state == ConnectionState.connected) {
          _logger.info(
            'Central connected to our peripheral: ${event.central.uuid}',
          );
          ...
          _updateConnectionInfo(
            ...
            isAdvertising: _connectionManager
                .isAdvertising, // Use connection manager's advertising state
          );
          ...
          if (_connectedCentral?.uuid == event.central.uuid) {
            ...
            _updateConnectionInfo(
              ...
              otherUserName: null,
              statusMessage: _connectionManager.isAdvertising
    ...
    peripheralManager.characteristicWriteRequested.listen((event) async {
      try {
        ...
        if (_connectedCentral == null) {
          _connectedCentral = event.central;
          _logger.info(
            'Setting connected central from write request: ${event.central.uuid}',
          );
        }
        ...
        if (!_peripheralHandshakeStarted &&
            _connectedCentral != null &&
            _connectedCharacteristic != null) {
  ...
  void _processPendingMessages() {
    ...
    _logger.info(
      '🔄 BUFFER: Processing ${_messageBuffer.length} buffered messages',
    );
    ...
    for (final bufferedMsg in bufferedMessages) {
      _logger.info(
        '🔄 BUFFER: Processing buffered message from ${bufferedMsg.timestamp}',
      );
      _processMessage(
        bufferedMsg.data,
  ...
  Future<void> _handleReceivedData(
    Uint8List data, {
    required bool isFromPeripheral,
    Central? central,
    ...
    try {
      ...
      if (_isHandshakeMessage(protocolMessage.type) &&
        ...
        _logger.info(
          '🔄 BUFFER: Handshake message arrived before coordinator ready - buffering ${protocolMessage.type}',
        );
        _messageBuffer.add(
          _BufferedMessage(
            data: data,
      ...
      if (protocolMessage.type == ProtocolMessageType.identity) {
        ...
        if (existingMessages.isNotEmpty) {
          ...
          _logger.info(
            'Contact restored from existing chat history: $displayName',
          );
        } else {
          _logger.info(
            'Identity received but not saving contact yet: $displayName',
          );
        }
        ...
        await chatsRepo.storeDeviceMapping(
      ...
      if (protocolMessage.type == ProtocolMessageType.persistentKeyExchange) {
        ...
        final persistentKey =
            protocolMessage.payload['persistentPublicKey'] as String?;
      ...
      if (protocolMessage.type == ProtocolMessageType.cryptoVerification) {
        ...
        if (challenge != null && testMessage != null) {
          _logger.info(
            '🔍 VERIFICATION: Crypto challenge received but challenges disabled - ignoring',
          );
        }
        return;
      }
      ...
      if (protocolMessage.type ==
          ProtocolMessageType.cryptoVerificationResponse) {
        final challenge = protocolMessage.cryptoVerificationResponseChallenge;
        final decryptedMessage =
            protocolMessage.cryptoVerificationResponseDecrypted;
        if (challenge != null && decryptedMessage != null) {
          _logger.info(
            '🔍 VERIFICATION: Crypto response received but challenges disabled - ignoring',
          );
        }
        return;
      }
    ...
    if (isChunk && chunk != null) {
      ...
      if (reassembledData != null) {
        ...
        try {
          ...
          if (_isHandshakeMessage(protocolMessage.type)) {
            _logger.info(
              '📦 Reassembled handshake message: ${protocolMessage.type}',
            );
            ...
            if (_handshakeCoordinator != null &&
                !_handshakeCoordinator!.isComplete &&
                !_handshakeCoordinator!.hasFailed) {
              await _handshakeCoordinator!.handleReceivedMessage(
                protocolMessage,
              );
              return;
            }
            ...
            if (_handshakeCoordinator == null) {
              _logger.info(
                '🔄 BUFFER: Reassembled handshake message before coordinator ready - buffering ${protocolMessage.type}',
              );
              _messageBuffer.add(
                _BufferedMessage(
          ...
          _logger.warning(
            'Reassembled message is not a valid protocol message: $e',
          );
          return;
        }
      } else {
  ...
  Future<void> _processMessage(
    ...
    if (_stateManager.theirEphemeralId != null) {
      ...
      final persistentKey = _stateManager.getPersistentKeyFromEphemeral(
    ...
    if (content != null) {
      ...
      print(
        '🟢 Number of stream listeners: ${_messagesController?.hasListener ?? false}',
      );
      ...
      if (_stateManager.currentSessionId != null &&
          _stateManager.otherUserName != null) {
        final contact = await _stateManager.getContact(
        ...
        if (contact == null) {
          ...
          _logger.info(
            'Contact saved on first message received: ${_stateManager.otherUserName} at low security',
          );
        }
      }
  ...
  Future<void> startScanning({
    ...
    _logger.info(
      '🔍 [SCAN-DEBUG] isPeripheralMode: ${_stateManager.isPeripheralMode}',
    );
    _logger.info(
    ...
    try {
      ...
      if (_authoritativeAdvertisingState && _stateManager.isPeripheralMode) {
        ...
        _logger.info(
          '🔧 DUAL-ROLE: Advertising state: $_authoritativeAdvertisingState, Scanning state: $_isDiscoveryActive',
        );
        _updateConnectionInfo(
          isAdvertising: true,
          isScanning: true,
    ...
    try {
      ...
      _handshakeCoordinator = HandshakeCoordinator(
        ...
        onHandshakeSuccess: (peerEphemeralId) async {
          ...
          if (AppCore.instance.isInitialized) {
            await AppCore.instance.messageQueue.flushQueueForPeer(
              peerEphemeralId,
            );
            _logger.info('✅ PHASE 1: Queue flush completed for peer');
          } else {
            _logger.warning(
              '⚠️ PHASE 1: AppCore not initialized - skipping queue flush',
            );
          }
        },
  ...
  Future<void> _onHandshakeComplete(
    ...
    if (noisePublicKey != null) {
      ...
      _logger.warning(
        '⚠️ No Noise public key provided - messages will be unencrypted',
      );
    }
    ...
    if (_handshakeCoordinator != null && !_handshakeCoordinator!.hasFailed) {
      ...
      _logger.warning(
        '⚠️ Handshake coordinator failed during completion - skipping UI update to ready state',
      );
    }
  }
  ...
  void _handleSpyModeDetected(SpyModeInfo info) {
    _logger.info(
      '🕵️ SPY MODE DETECTED: User is chatting with ${info.contactName} anonymously',
    );
  ...
  void _handleIdentityRevealed(String contactName) {
    _logger.info(
      '🕵️ IDENTITY REVEALED: Contact $contactName now knows your identity',
    );
  ...
  Future<bool> sendMessage(
    ...
    return await _messageHandler.sendMessage(
      ...
      onMessageOperationChanged: (inProgress) =>
          _connectionManager.setMessageOperationInProgress(inProgress),
    );
  }
  ...
  Future<void> _sendIdentityExchange() async {
    ...
    try {
      ...
      if (_stateManager.myUserName == null ||
        ...
        await _stateManager.loadUserName();
        print(
          '🐛 DEBUG NAME: After reload in identity exchange: "${_stateManager.myUserName}"',
        );
      }
  ...
  void stopConnectionMonitoring() =>
      _connectionManager.stopConnectionMonitoring();
  void setHandshakeInProgress(bool inProgress) =>
      _connectionManager.setHandshakeInProgress(inProgress);
  ...
  Future<bool> attemptIdentityRecovery() async {
    ...
    try {
      ...
      if (recovered) {
        ...
        _logger.info(
          '  - Type: ${_stateManager.isPaired ? "PAIRED" : "UNPAIRED"}',
        );
  ...
  void _onBluetoothBecameReady() {
    ...
    if (!_meshNetworkingStarted) {
      ...
      _meshNetworkingStarted =
          true; // ✅ FIX: Set immediately to prevent duplicate starts from other listeners
      Future.delayed(Duration(milliseconds: 500), () async {
        try {
          await _connectionManager.startMeshNetworking(
            onStartAdvertising: () async {
              _logger.info(
                '📡 [DEFERRED-START] Starting peripheral mode via AdvertisingManager...',
              );
              await startAsPeripheral();
              _logger.info(
                '✅ [DEFERRED-START] Peripheral mode started successfully',
              );
            },
          );
          _logger.info(
          ...
          _meshNetworkingStarted =
              false; // Reset on failure so it can be retried
        }
      });
      return;
    }
      ...
      Future.delayed(Duration(milliseconds: 500), () async {
        try {
          await _connectionManager.startMeshNetworking(
            onStartAdvertising: () async {
              _logger.info(
                '📡 [LOST-ADV-RESTART] Starting peripheral mode via AdvertisingManager...',
              );
              await startAsPeripheral();
              _logger.info(
                '✅ [LOST-ADV-RESTART] Peripheral mode started successfully',
              );
            },
          );
        } catch (e) {
  ...
  void _onBluetoothBecameUnavailable() {
    ...
    if (hasActiveSession) {
      ...
      _logger.fine(
        '🔵 No active session - skipping session clear (already disconnected)',
      );
  ...
  void _onBluetoothInitializationRetry() {
    ...
    _updateConnectionInfo(statusMessage: 'Checking Bluetooth status...');
  }
  ...
  Future<void> startScanningWithValidation({
    ...
    if (!_bluetoothStateMonitor.isBluetoothReady) {
      ...
      _logger.warning(
        '🔵 Cannot start scanning - Bluetooth not ready: $currentState',
      );
      ...
      _updateConnectionInfo(isScanning: false, statusMessage: errorMessage);
  ...
  Future<void> startAsPeripheralWithValidation() async {
    ...
    if (!_bluetoothStateMonitor.isBluetoothReady) {
      final currentState = _bluetoothStateMonitor.currentState;
      _logger.warning(
        '🔵 Cannot start peripheral mode - Bluetooth not ready: $currentState',
      );
      ...
      _updateConnectionInfo(isAdvertising: false, statusMessage: errorMessage);
