...
class BLEStateManager {
  ...
  bool _cryptoVerificationInProgress = false;
  String? _currentVerificationChallenge;
  final Map<String, bool> _deviceVerificationStatus = {};
  ...
  Function(String, String)? onMutualConsentRequired; // New callback for mutual consent prompt
  
  // Crypto verification callbacks
  Function(ProtocolMessage)? onSendCryptoVerification;
  Function(ProtocolMessage)? onSendCryptoVerificationResponse;
  Function(bool)? onCryptoVerificationCompleted;
...
Future<void> initializeContactFlags() async {
    ...
    // Check if crypto verification is needed
    if (_otherDevicePersistentId != null) {
      await _checkAndInitiateCryptoVerification();
    }
  }
  
  /// Check if crypto verification is needed and initiate it
  Future<void> _checkAndInitiateCryptoVerification() async {
    if (_otherDevicePersistentId == null) return;
    
    try {
      _logger.info('🔍 VERIFICATION: Checking if verification needed for ${_otherDevicePersistentId!.substring(0, 16)}...');
      
      final isRequired = await CryptoVerificationManager.isVerificationRequired(_otherDevicePersistentId!, _contactRepository);
      
      if (isRequired && !_cryptoVerificationInProgress) {
        _logger.info('🔍 VERIFICATION: Initiating crypto verification for new/unverified connection');
        await _initiateCryptoVerification();
      } else if (!isRequired) {
        _logger.info('🔍 VERIFICATION: Device already verified, skipping verification');
        _deviceVerificationStatus[_otherDevicePersistentId!] = true;
      } else {
        _logger.info('🔍 VERIFICATION: Verification already in progress');
      }
    } catch (e) {
      _logger.warning('🔍 VERIFICATION: Failed to check verification requirements: $e');
    }
  }
  
  /// Initiate crypto verification process
  Future<void> _initiateCryptoVerification() async {
    if (_otherDevicePersistentId == null) return;
    
    try {
      _logger.info('🔍 VERIFICATION: Starting verification process');
      
      _cryptoVerificationInProgress = true;
      
      // Generate verification challenge
      final challengeMessage = await CryptoVerificationManager.generateVerificationChallenge(
        _otherDevicePersistentId!,
        _contactRepository
      );
      
      if (challengeMessage == null) {
        _logger.severe('🔍 VERIFICATION: Failed to generate challenge message');
        await _handleVerificationFailure('Failed to generate challenge');
        return;
      }
      
      _currentVerificationChallenge = challengeMessage.cryptoVerificationChallenge;
      
      // Set verification timeout (30 seconds)
      _verificationTimeout?.cancel();
      _verificationTimeout = Timer(Duration(seconds: 30), () {
        _logger.warning('🔍 VERIFICATION: Timeout waiting for verification response');
        _handleVerificationFailure('Verification timeout');
      });
      
      // Send challenge via callback
      _logger.info('🔍 VERIFICATION: Sending challenge to remote device');
      onSendCryptoVerification?.call(challengeMessage);
      
    } catch (e) {
      _logger.severe('🔍 VERIFICATION: Failed to initiate verification: $e');
      await _handleVerificationFailure('Initiation failed: $e');
    }
  }
  
  /// Handle received crypto verification challenge
  Future<void> handleCryptoVerificationChallenge(String challenge, String testMessage) async {
    if (_otherDevicePersistentId == null) return;
    
    try {
      _logger.info('🔍 CHALLENGE: Received verification challenge from ${_otherDevicePersistentId!.substring(0, 16)}...');
      
      // Handle the challenge using the verification manager
      final responseMessage = await CryptoVerificationManager.handleVerificationChallenge(
        challenge,
        testMessage,
        _otherDevicePersistentId!,
        _contactRepository
      ...
      if (responseMessage == null) {
        _logger.severe('🔍 CHALLENGE: Failed to generate response to challenge');
        return;
      }
      
      // Send response via callback
      _logger.info('🔍 CHALLENGE: Sending verification response');
      onSendCryptoVerificationResponse?.call(responseMessage);
      
      // If response was successful, mark as verified
      if (responseMessage.cryptoVerificationSuccess) {
        _logger.info('🔍 CHALLENGE: ✅ Challenge handled successfully');
        _deviceVerificationStatus[_otherDevicePersistentId!] = true;
      }
      
    } catch (e) {
      _logger.severe('🔍 CHALLENGE: Failed to handle verification challenge: $e');
    }
  }
  
  /// Handle received crypto verification response
  Future<void> handleCryptoVerificationResponse(
    String challenge,
    ...
    try {
      _logger.info('🔍 RESPONSE: Received verification response from ${_otherDevicePersistentId!.substring(0, 16)}...');
      
      // Clear timeout since we received a response
      _verificationTimeout?.cancel();
      
      // Verify the challenge matches what we sent
      if (challenge != _currentVerificationChallenge) {
        _logger.severe('🔍 RESPONSE: Challenge mismatch - possible security issue');
        await _handleVerificationFailure('Challenge mismatch');
        return;
      }
      
      // Process the response using verification manager
      final verificationSuccess = await CryptoVerificationManager.processVerificationResponse(
        challenge,
        decryptedMessage,
        success,
        results,
      ...
      if (verificationSuccess) {
        _logger.info('🔍 RESPONSE: ✅ Crypto verification completed successfully');
        await _handleVerificationSuccess();
      } else {
        _logger.severe('🔍 RESPONSE: ❌ Crypto verification failed');
        await _handleVerificationFailure('Verification response processing failed');
      }
      
    } catch (e) {
      _logger.severe('🔍 RESPONSE: Failed to handle verification response: $e');
      await _handleVerificationFailure('Response handling exception: $e');
    }
  }
  ...
  Future<void> _handleVerificationSuccess() async {
    ...
    try {
      _cryptoVerificationInProgress = false;
      _currentVerificationChallenge = null;
      _deviceVerificationStatus[_otherDevicePersistentId!] = true;
      
      _logger.info('🔍 SUCCESS: ✅ Device crypto verification completed successfully');
      _logger.info('🔍 SUCCESS: Device is now verified and can use secure features');
      
      // Notify UI of successful verification
      onCryptoVerificationCompleted?.call(true);
      
      // Trigger contact operations that were waiting for verification
      await _proceedWithSecureOperations();
      
    } catch (e) {
      _logger.severe('🔍 SUCCESS: Failed to handle verification success: $e');
    }
  }
  ...
  Future<void> _handleVerificationFailure(String reason) async {
    ...
    try {
      ...
      _currentVerificationChallenge = null;
      _verificationTimeout?.cancel();
      _deviceVerificationStatus[_otherDevicePersistentId!] = false;
      
      _logger.severe('🔍 FAILURE: ❌ Crypto verification failed: $reason');
      _logger.warning('🔍 FAILURE: Device will have limited security features');
      
      // Mark device as unverified in repository
      await _contactRepository.updateContactSecurityLevel(_otherDevicePersistentId!, SecurityLevel.low);
      
      // Notify UI of verification failure
      onCryptoVerificationCompleted?.call(false);
      
      // Continue with basic operations but limited security
      await _proceedWithLimitedOperations();
      
    } catch (e) {
      _logger.severe('🔍 FAILURE: Failed to handle verification failure: $e');
    }
  }
  
  /// Proceed with secure operations after successful verification
  Future<void> _proceedWithSecureOperations() async {
    _logger.info('🔍 SECURE: Proceeding with full secure operations');
    
    // Enable high-security features
    // Contact operations, ECDH encryption, etc. can now proceed
    
    // Continue with normal contact synchronization
    await initializeContactFlags();
  }
  
  /// Proceed with limited operations after verification failure
  Future<void> _proceedWithLimitedOperations() async {
    _logger.info('🔍 LIMITED: Proceeding with limited security operations');
    
    // Basic operations only - global encryption level
    // Contact operations still available but with lower security
    
    // Continue with normal contact synchronization but with warnings
    await initializeContactFlags();
  }
  
  /// Check if device is verified
  bool isDeviceVerified(String? publicKey) {
    if (publicKey == null) return false;
    return _deviceVerificationStatus[publicKey] ?? false;
  }
  
  /// Get current verification status
  bool get isCryptoVerificationInProgress => _cryptoVerificationInProgress;
