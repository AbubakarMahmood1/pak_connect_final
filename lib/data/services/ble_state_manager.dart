...
Future<void> _deleteIntroHintAfterConnection(String displayName, String publicKey) async {
  try {
    ...
    for (final hint in scannedHints.values) {
      if (hint.displayName == displayName) {
        ...
        _logger.info('🗑️ PRIVACY: Deleted intro hint after connection: ${hint.hintHex} ($displayName)');
        _logger.info('   Reason: Intro hints are temporary - prevents identity linkage across sessions');
        return;
      }
    }

    _logger.fine('ℹ️ No intro hint found to delete for $displayName (may not be QR-based connection)');
  } catch (e, stackTrace) {
