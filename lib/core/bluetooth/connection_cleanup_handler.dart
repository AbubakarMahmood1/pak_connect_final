...
class ConnectionCleanupHandler {
  ...
  void registerConnection({
    ...
    _logger.info(
      '📥 Registering connection: ${_formatAddress(deviceAddress)} (${isClient ? "client" : "server"})',
    );
  ...
  Future<void> handleDisconnect({
    ...
    if (connection == null) {
      _logger.warning(
        '⚠️ Disconnect for unknown device: ${_formatAddress(deviceAddress)}',
      );
      return;
    }
    ...
    _logger.info(
      '✅ Disconnect cleanup complete: ${_formatAddress(deviceAddress)}',
    );
    _logger.fine('🧹 Active connections: ${_activeConnections.length}');
  }
  ...
  void _cleanupExpiredPendingConnections() {
    ...
    if (expiredIds.isNotEmpty) {
      ...
      _logger.info(
        '🗑️ Cleaned up ${expiredIds.length} expired pending connections',
      );
    }
  }
...
class _PendingConnection {
  ...
  _PendingConnection({required this.deviceId, required this.lastAttempt});
}
