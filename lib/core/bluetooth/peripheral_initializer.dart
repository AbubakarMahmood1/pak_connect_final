...
class PeripheralInitializer {
  ...
  Future<bool> waitUntilReady({
    ...
    try {
      ...
      final duration = DateTime.now()
          .difference(_initStartTime!)
          .inMilliseconds;
      _logger.info('✅ Peripheral ready after ${duration}ms');
      return true;
    } on TimeoutException {
      _logger.warning(
        '⏱️ Peripheral initialization timeout after ${timeout.inSeconds}s',
      );
      return false;
    } catch (e) {
  ...
  Future<bool> safelyStartAdvertising(
    ...
    _logger.info(
      '   - Manufacturer data: ${advertisement.manufacturerSpecificData.length} entries',
    );
    ...
    _logger.info(
      '🔍 [PERIPH-DEBUG] skipIfAlreadyAdvertising: $skipIfAlreadyAdvertising',
    );
    _logger.info('🔍 [PERIPH-DEBUG] ========================================');

    // Step 1: Ensure peripheral is ready
    _logger.info(
      '🔍 [PERIPH-DEBUG] STEP 1: Checking if peripheral is ready...',
    );
    final isReady = await waitUntilReady(timeout: timeout);
    if (!isReady) {
      _logger.severe('❌❌❌ [PERIPH-DEBUG] PERIPHERAL NOT READY! ❌❌❌');
      _logger.severe(
        '❌ [PERIPH-DEBUG] Cannot start advertising - peripheral initialization failed',
      );
      return false;
    }
    ...
    _logger.info(
      '🔍 [PERIPH-DEBUG] STEP 2: Checking if already advertising...',
    );
    if (skipIfAlreadyAdvertising) {
      try {
        // Try to stop advertising first - if it fails, we weren't advertising
        _logger.info(
          '🔍 [PERIPH-DEBUG] Attempting to stop previous advertising...',
        );
        ...
        _logger.info(
          '🔍 [PERIPH-DEBUG] No active advertising to stop (this is OK)',
        );
      }
    } else {
      _logger.info(
        '🔍 [PERIPH-DEBUG] Skipping stop check (skipIfAlreadyAdvertising=false)',
      );
    }
