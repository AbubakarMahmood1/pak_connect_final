...
class SmartHandshakeManager {
  ...
  Future<HandshakeInitiationResult> initiateHandshake(String peerID) async {
    ...
    if (pattern == NoisePattern.kk && _shouldSkipKK(peerID)) {
      _logger.warning('⚠️ Skipping KK for $peerID due to previous failures, using XX');
  ...
  Future<HandshakeInitiationResult> _initiateWithPattern(
    ...
    try {
      ...
      _logger.info('✅ Initiated ${pattern.name.toUpperCase()} handshake with $peerID');
      
      return HandshakeInitiationResult(
        message: message,
        patternUsed: pattern,
      );
      
    } catch (e) {
      _logger.severe('❌ Failed to initiate ${pattern.name.toUpperCase()} handshake: $e');
      rethrow;
    }
  }
    ...
    if (error is NoiseHandshakeException) {
      ...
      if (error.shouldFallbackToXX) {
        ...
        if (_kkFailureCount[peerID]! >= _maxKKFailures) {
          _logger.warning('⬇️ Too many KK failures (${ _kkFailureCount[peerID]}) - contact may have reset device');
        ...
        _logger.info('⏸️ KK handshake failed due to timeout/network - will retry KK later');
        return false; // Do NOT retry with XX
      }
    }
    
    // Unknown error type - assume transient (DO NOT fallback)
    _logger.warning('⚠️ Unknown handshake error type - assuming transient, no fallback');
    return false;
  }
  ...
  Future<void> _considerDowngrade(String peerID) async {
    ...
    if (contact.securityLevel != SecurityLevel.low) {
      _logger.warning('⬇️ Downgrading $peerID from ${contact.securityLevel.name} to LOW');
      _logger.warning('   Reason: Repeated KK handshake failures suggest peer reset device');
  ...
  void markHandshakeSuccess(String peerID, NoisePattern pattern) {
    ...
    _logger.info('✅ Handshake success with $peerID using ${pattern.name.toUpperCase()}');
  }
