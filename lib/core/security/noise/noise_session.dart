...
class NoiseSession {
  ...
  NoiseSession({
    ...
    if (pattern == NoisePattern.kk && remoteStaticPublicKey == null) {
      throw ArgumentError(
        'KK pattern requires remoteStaticPublicKey parameter',
      );
    }
    if (pattern == NoisePattern.kk && remoteStaticPublicKey!.length != 32) {
      throw ArgumentError(
        'remoteStaticPublicKey must be 32 bytes for KK pattern',
      );
    }

    _logger.info(
      '[$peerID] Created ${pattern.name.toUpperCase()} session as ${isInitiator ? "INITIATOR" : "RESPONDER"}',
    );
  }
  ...
  Future<Uint8List> startHandshake() async {
    ...
    _logger.info(
      '[$peerID] Starting ${pattern.name.toUpperCase()} handshake as INITIATOR',
    );
  ...
  Future<Uint8List?> processHandshakeMessage(Uint8List message) async {
    _logger.fine(
      '[$peerID] Processing handshake message (${message.length} bytes)',
    );
  ...
  Future<Uint8List?> _processHandshakeMessageXX(Uint8List message) async {
    ...
    if (isInitiator) {
      ...
      if (messageIndex == 1 && message.length == _xxMessage2Size) {
        ...
        _logger.fine(
          '[$peerID] Initiator sending XX message 3 (${response.length} bytes)',
        );
      ...
      if (messageIndex == 0 && message.length == _xxMessage1Size) {
        ...
        _logger.fine(
          '[$peerID] Responder sending XX message 2 (${response.length} bytes)',
        );
        return response;
  ...
  Future<Uint8List?> _processHandshakeMessageKK(Uint8List message) async {
    ...
    if (isInitiator) {
      ...
      if (messageIndex == 0 && message.length == _kkMessage1Size) {
        ...
        _logger.fine(
          '[$peerID] Responder sending KK message 2 (${response.length} bytes)',
        );
  ...
  bool _isValidNonce(int receivedNonce) {
    ...
    if (receivedNonce + _replayWindowSize <= _highestReceivedNonce) {
      _logger.warning(
        '[$peerID] Nonce too old: $receivedNonce (highest: $_highestReceivedNonce)',
      );
      return false;
    }
  ...
  void destroy() {
    ...
    _remoteStaticPublicKeyForKK?.fillRange(
      0,
      _remoteStaticPublicKeyForKK.length,
      0,
    );
