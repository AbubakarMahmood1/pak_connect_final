...
class DeviceTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    (bleService.connectedDevice?.uuid == device.uuid || 
     (bleService.otherUserName != null && bleService.otherUserName!.isNotEmpty));
    
    // Watch name changes to determine if names are exchanged
      ...
      child: ListTile(
        ...
        title: Text(
          hasNameExchange && isConnected
              ? nameAsync.maybeWhen(data: (name) => name!, orElse: () => 'Device ${device.uuid.toString().substring(0, 8)}...')
