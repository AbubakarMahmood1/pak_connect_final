...
class AutoArchiveScheduler {
  ...
  static Future<int> _checkAndArchiveInactiveChats() async {
    try {
      ...
      final archiveAfterDays = await prefs.getInt(PreferenceKeys.archiveAfterDays);
      final cutoffDate = DateTime.now().subtract(Duration(days: archiveAfterDays));
      
      _logger.info('Checking for chats inactive since: $cutoffDate ($archiveAfterDays days ago)');
      ...
      for (final chat in allChats) {
        ...
        if (chat.lastMessageTime!.isAfter(cutoffDate)) {
          _logger.fine('Skipping ${chat.contactName}: active within $archiveAfterDays days');
          continue;
        }
        
        // Calculate days inactive
        final daysInactive = DateTime.now().difference(chat.lastMessageTime!).inDays;
        
        _logger.info('Found inactive chat: ${chat.contactName} ($daysInactive days inactive)');
        ...
        try {
          ...
          if (result.success) {
            ...
            _logger.info('✅ Auto-archived: ${chat.contactName} (inactive for $daysInactive days)');
          } else {
            _logger.warning('❌ Failed to auto-archive ${chat.contactName}: ${result.message}');
          }
        } catch (e) {
