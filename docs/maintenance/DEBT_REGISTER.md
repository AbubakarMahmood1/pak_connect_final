...
|---|---|---|---|---|
| DEBT-CI-001 | P1 | Now/PR #73 | Historical public-main Flutter run `29215130687` failed two parallel suites with SQLite `database is locked`. Commit `569ff9a` preserves suite-specific database names and the local full suite is green. The branch is pushed through normal PR #73, and protected `main` now strictly requires `test`, Android CodeQL and Actions CodeQL | Close only after the final exact PR head passes all three required contexts, PR #73 merges normally, and the resulting `main` is verified |
