...
|---|---|---|---|---|
| DEBT-CI-001 | P1 | Closed 2026-07-29 | Historical public-main Flutter run `29215130687` failed two parallel suites with SQLite `database is locked`. Commit `569ff9a` preserves suite-specific database names. PR #73 exact head `d551f00` passed Flutter run `30399126498` and CodeQL run `30399126495`, then merged normally as `3d9bcdb`. Protected `main` strictly requires `test`, Android CodeQL and Actions CodeQL | Reopen only if a required context is removed, a protected-main run fails, or another parallel database collision appears |
